...
void do_makegoggles( CHAR_DATA *ch, char *argument )
{
    ...
    { 
    	default:
                ...
                {
                  send_to_char( "&RSintaxis: fabricargafas <infrarojos/aumento> <nombre>\n\r&w", ch);
                  return;   
                }
                ...
                if ( !checkduraplast )
                {
                   send_to_char( "&RNecesitas una pieza de duraplástico.\n\r", ch);
                   return;
                }
                 if ( !checkcirc )
                {
    		...
    		{
			if(!str_cmp(arg, "infrarojos"))
    		   send_to_char( "&Gcomienzas el largo proceso de crear unas gafas de infrarojos.\n\r", ch);
            else
               send_to_char( "&GComienzas el largo proceso de construir unas gafas de aumento.\n\r", ch);
    		   act( AT_PLAIN, "$n coge su caja de herramientas y se pone a trabajar en algo.", ch,
    ...
    {
       send_to_char( "&RActivas las gafas y la lente se rompe.\n\r", ch);
       learn_from_failure( ch, gsn_makegoggles );
       return;
    }
  ...
  if(!str_cmp(arg, "infrared"))
   ...
   if(!str_cmp(arg, "infrarojos"))
    send_to_char( "&GTerminas tus gafas de infrarojos.&w\n\r", ch);
   else
    send_to_char( "&GTerminas tus gafas de aumento.&w\n\r", ch);

    act( AT_PLAIN, "$n construye unas gafas.", ch,
         NULL, argument , TO_ROOM );
...
void do_link(CHAR_DATA *ch, char *argument)
{
 ...
 {
  send_to_char("&RDebes estar en el asiento de armas para hacer eso.\n\r", ch);
  return;
 }
 ...
 {
  send_to_char("Sintaxis: enlazar <primario/secundario/lanzadores>\n\r", ch);
  return;
 }

 if(!str_prefix(argument, "primario"))
 {
	if(ship->primaryType == B_NONE)
	{
	 send_to_char("&REsta nave no está equipada con un sistema de armas primario.\n\r", ch);
	 return;
	}
	...
	if(x <= 1)
	{
	 send_to_char("El sistema de armas primario solo puede disparar una vez.\n\r", ch);
	 return;
        }

	ship->primaryLinked = TRUE;
	ch_printf(ch, "Sistemas primarios de armas enlazados. Listos para disparar.\n\r");
	return;
 }
 if(!str_prefix(argument, "secundario"))
 {
	if(ship->secondaryType == B_NONE)
	{
	 send_to_char("&REsta nave no está equipada con un sistema de armas secundario.\n\r", ch);
	 return;
	}
	...
	if(x <= 1)
	{
	 send_to_char("El sistema de armas secundario solo puede disparar una vez.\n\r", ch);
	 return;
	}

	ship->secondaryLinked = TRUE;
	ch_printf(ch, "Sistemas de armas secundarios enlazados. Listos para disparar.\n\r");
	return;
 }
 if(!str_prefix(argument, "lanzadores"))
 {
  if(!ship->maxmissiles && !ship->maxtorpedos && !ship->maxrockets)
  {
   send_to_char("Esta nave no tiene  lanzadores instalados.\n\r", ch);
   return;
  }
      ...
      (ship->maxrockets && ship->maxtorpedos) )
  {
   ...
   ch_printf(ch, "Lanzadores enlazados. Listos para disparar.\n\r");
   return;
  }
  else
  {
   ch_printf(ch, "Esta nave solo tiene un tipo de lanzador. el enlace no es necesario.\n\r");
   return;
  }
 }
}
...
void do_unlink(CHAR_DATA *ch, char *argument)
{
 ...
 if( (ship = ship_from_gunseat(ch->in_room->vnum)) == NULL )
 {
  send_to_char("&RDebes estar en el asiento de armas de la nave para hacer esto.\n\r", ch);
  return;
 }
 
 if(str_cmp(argument, "primario") && str_cmp(argument, "secundario")
  && str_cmp(argument,"lanzadores") && str_cmp(argument, "todos"))
 {
  send_to_char("Sintaxis: desenlazar <primario/secundario/lanzadores/todos>\n\r", ch);
  return;
 }
 ...
 if(!str_prefix(argument, "primario"))
	{
	 if(ship->primaryCount == 0)
		{
		 send_to_char("Esta nave no tiene un sistema primario de armas.\n\r", ch);
		 return;
		}
	 if(ship->primaryLinked == FALSE)
		{
		 send_to_char("Los sistemas primarios de armas de la nave ya están desenlazados.\n\r", ch);
		 return;
		}
	 ship->primaryLinked = FALSE;
	 send_to_char("Desenlazas los sistemas primarios de armas de la nave.\n\r", ch);
	 act(AT_PLAIN, "$n pulsa algunos botones en la consola de la nave.\n\r", ch, NULL, NULL, TO_ROOM);
	 return;
	}
 if(!str_prefix(argument, "secundario"))
	{
	 if(ship->secondaryCount == 0)
		{
		 send_to_char("Esta nave no tiene un sistema de armas secundario.\n\r", ch);
		 return;
		}
	 if(ship->secondaryLinked == FALSE)
		{
		 send_to_char("El sistema de armas secundario de la nave no está enlazado.\n\r", ch);
		 return;
		}
	 ship->secondaryLinked = FALSE;
	 send_to_char("Desenlazas los sistemas de armas secundarios de la nave.\n\r", ch);
	 act(AT_PLAIN, "$n pulsa algunos botones en la consola de la nave.\n\r", ch, NULL, NULL, TO_ROOM);
	 return;
	}
 if(!str_prefix(argument, "lanzadores"))
	{
	 if(ship->warheadLinked == FALSE)
		{
		 send_to_char("Los lanzadores no están enlazados.\n\r", ch);
		 return;
		}
	 ship->warheadLinked = FALSE;
	 send_to_char("Desenlazas los lanzadores.\n\r", ch);
	 act(AT_PLAIN, "$n pulsa algunos botones en la consola de la nave.\n\r", ch, NULL, NULL, TO_ROOM);
	 return;
	}
 if(!str_prefix(argument, "todos"))
	{
	 if(ship->warheadLinked == FALSE && ship->primaryLinked == FALSE && ship->secondaryLinked == FALSE)
		{
		 send_to_char("No hay sistemas enlazados en la nave.\n\r", ch);
		 return;
		}
	 if(ship->primaryLinked == TRUE)
		{
		 ship->primaryLinked = FALSE;
		 send_to_char("Sistemas de armas primarios desenlazados.\n\r", ch);
		}
	 if(ship->secondaryLinked == TRUE)
		{
		 ship->secondaryLinked = FALSE;
		 send_to_char("Sistemas secundarios de armas de la nave desenlazados.\n\r", ch);
		}
	 if(ship->warheadLinked == TRUE)
		{
		 ship->warheadLinked = FALSE;
		 send_to_char("Lanzadores de la nave desenlazados.\n\r", ch);
		}
		return;
	}
}
...
void do_barrel_roll(CHAR_DATA *ch, char *argument)
{
 ...
 {
  send_to_char("&RDebes estar en el asiento del copiloto para ejecutar esta maniobra.\n\r", ch);
  return;
 }

 if(ship->manuever < 70)
 {
	 send_to_char("la nave no es suficientemente maniobrable para realizar un tonel.\n\r", ch);
	 return;
 }
 if (ship->shipstate == SHIP_HYPERSPACE)
 {
      send_to_char("&Rsolo puedes hacer eso en espacio real.\n\r",ch);
      return;   
 }              
 if (! ship->starsystem )
 {
 ...
 switch(ch->substate)
 {
  default:
	...
	if(number_percent() > chance)
	{
		send_to_char("&RIntentas hacer un tonel pero fallas horriblemente.\n\r", ch);
		return;
	}

    if(ship->juking == TRUE)
    {
	  send_to_char("&YDejas de realizar la maniobra y realizas un tonel.\n\r", ch);
	  sprintf(buf, "%s deja de realizar su maniobra.", ship->name);
	  echo_to_system(AT_YELLOW, ship, buf, NULL);
	  ship->juking = FALSE;
    }
	ship->rolling = TRUE;
	send_to_char("&GLa nave comienza a rodar...\n\r", ch);
	act(AT_PLAIN, "$n realiza un tonel.", ch, NULL, NULL, TO_ROOM);
	sprintf(buf, "%s realiza un tonel.", ship->name);
	echo_to_system(AT_YELLOW, ship, buf, NULL);
  ...
  case SUB_TIMER_DO_ABORT:
    ...
    send_to_char("&YVuelves al patrón de vuelo original.\n\r", ch);
	act(AT_PLAIN, "$n vuelve a su patrón de vuelo original.\n\r", ch, NULL, NULL, TO_ROOM);
	sprintf(buf, "%s vuelve a su patrón de vuelo original.", ship->name);
	echo_to_system(AT_YELLOW, ship, buf, NULL);
	ship->rolling = FALSE;
	return;
 }
	...
	ch->substate = SUB_NONE;
	if(( ship = ship_from_cockpit(ch->in_room->vnum) ) == NULL)  return;
	send_to_char("&Yvuelves a tu patrón de vuelo original.\n\r", ch);
	act(AT_PLAIN, "$n vuelve a su patrón de vuelo original.\n\r", ch, NULL, NULL, TO_ROOM);
	sprintf(buf, "%s vuelve a su patrón de vuelo original.", ship->name);
	echo_to_system(AT_YELLOW, ship, buf, NULL);
	ship->rolling = FALSE;
...
void do_juke(CHAR_DATA *ch, char *argument)
{
 ...
 if(ship->manuever < 75)
 {
	 send_to_char("Esta nave no puede hacer picados.\n\r", ch);
	 return;
 }
 if (ship->shipstate == SHIP_HYPERSPACE)
 {
 ...
 switch(ch->substate)
 {
  default:
	...
	if(number_percent() > chance)
	{
		send_to_char("&RIntentas hacer un picado pero fallas horriblemente.\n\r", ch);
		return;
	}

    if(ship->rolling == TRUE)
    {
	  send_to_char("&YDejas de hacer un tonel y comienzas un picado.\n\r", ch);
	  sprintf(buf, "%s stops rolling.", ship->name);
	  echo_to_system(AT_YELLOW, ship, buf, NULL);
	  ship->rolling = FALSE;
    }
	ship->juking = TRUE;
	send_to_char("&GRealizaas la maniobra y la nave comienza a realizar un picado.\n\r", ch);
	act(AT_PLAIN, "$n realiza un picado.", ch, NULL, NULL, TO_ROOM);
	sprintf(buf, "%s realiza un picado.", ship->name);
	echo_to_system(AT_YELLOW, ship, buf, NULL);
  ...
  case SUB_TIMER_DO_ABORT:
	...
	ch->substate = SUB_NONE;
	if(( ship = ship_from_cockpit(ch->in_room->vnum) ) == NULL)  return;
	send_to_char("vuelves a tu patrón de vuelo original.\n\r", ch);
	act(AT_PLAIN, "$n vuelve a su patrón de vuelo original.\n\r", ch, NULL, NULL, TO_ROOM);
	sprintf(buf, "%s vuelve a su patrón de vuelo original.", ship->name);
	echo_to_system(AT_YELLOW, ship, buf, NULL);
	ship->juking = FALSE;
