...
bool spec_customs_smut( CHAR_DATA *ch )
{
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_SMUT)
	    {
	       ...
	       {
	          sprintf( buf , "%es contrabando ilegal. Debo confiscar eso.", obj->short_descr );
                  do_say( ch , buf );
                  if ( obj->wear_loc != WEAR_NONE )
                     remove_obj( victim, obj->wear_loc, TRUE );
                  separate_obj( obj );
    		  obj_from_char( obj );
    		  act( AT_ACTION, "$n confisca $p a $N.", ch, obj, victim, TO_NOTVICT );
    		  act( AT_ACTION, "$n te confisca $p.",   ch, obj, victim, TO_VICT    );
	       ...
	       { 
                  ch_exp = UMIN( obj->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                  ch_printf( victim, "Recibes %ld experiencia en contrabando gracias a que has conseguido ocultar %s.\n\r " , ch_exp , obj->short_descr );
                  gain_exp( victim, ch_exp , SMUGGLING_ABILITY );
       
	          act( AT_ACTION, "$n mira a $N sospechosamente.", ch, NULL, victim, TO_NOTVICT );
    		  act( AT_ACTION, "$n te mira sospechosamente.",   ch, NULL, victim, TO_VICT  );
                  separate_obj( obj );
    		  SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
	          return TRUE;
	       }
	       ...
	       {
                  ch_exp = UMIN( obj->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                  ch_printf( victim, "Recibes %ld experiencia en contrabando por contrabandear con %s.\n\r " , ch_exp, obj->short_descr );
                  gain_exp( victim, ch_exp , SMUGGLING_ABILITY );
                  separate_obj( obj );       
	          SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
	          return TRUE;
	        ...
	        for ( content = obj->first_content; content; content = content->next_content )
	        {
	            ...
	            && !IS_SET( content->extra_flags , ITEM_CONTRABAND ) )
	            {
	               ch_exp = UMIN( content->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                     ch_printf( victim, "Recibes %ld puntos de experiencia en contrabando por contrabandear con %s.\n\r " , ch_exp , content->short_descr );
                       gain_exp( victim, ch_exp, SMUGGLING_ABILITY ); 
...
bool spec_customs_weapons( CHAR_DATA *ch )
{
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_WEAPON)
	    {
	       if ( victim != ch && can_see( ch, victim ) && can_see_obj( ch,obj ) )
	       {
	          sprintf( buf , "Las armas están prohibidas para los civiles. Debo confiscarte %s.", obj->short_descr );
                  do_say( ch , buf );
                  if ( obj->wear_loc != WEAR_NONE )
                    remove_obj( victim, obj->wear_loc, TRUE );
                  separate_obj( obj );
	       ...
	       { 
                  ch_exp = UMIN( obj->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                  ch_printf( victim, "Recibes %ld experiencia por contrabandear con %d.\n\r " , ch_exp, obj->short_descr );
                  gain_exp( victim, ch_exp, SMUGGLING_ABILITY );
		  ...
		  SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
	       ...
	       {
                  ch_exp = UMIN( obj->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                  ch_printf( victim, "Recibes %ld puntos de experiencia por contrabandear con %s.\n\r " , ch_exp , obj->short_descr);
                  gain_exp( victim, ch_exp , SMUGGLING_ABILITY);
                  separate_obj( obj );
	          SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
	          return TRUE;
	    ...
	    else if ( obj->item_type == ITEM_CONTAINER )
	    {
	        ...
	        for ( content = obj->first_content; content; content = content->next_content )
	        {
	            ...
	            {
	               ch_exp = UMIN( content->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                     ch_printf( victim, "Recibes %ld experiencia por contrabandear con %s.\n\r " , ch_exp , content->short_descr);
                       gain_exp( victim, ch_exp, SMUGGLING_ABILITY );
	               separate_obj( content );
                       SET_BIT( content->extra_flags , ITEM_CONTRABAND);
	               return TRUE;
