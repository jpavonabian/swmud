...
void do_bounties( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !count )
    {
        ...
        send_to_char( "Por el momento no hay ninguna recompensa.\n\r", ch );
	return;
    }
}
void do_rembounty( CHAR_DATA *ch, char *argument ){
	...
	if (bounty)
	  ...
	  send_to_char("¡Esa recompensa no existe!\n\r", ch);
	  return;
	}
	sprintf(buf, "¡La recompensa sobre %s ha sido eliminada!", argument);
...
void disintegration ( CHAR_DATA *ch , CHAR_DATA *victim , long amount )
{
    ...
    sprintf( buf, "Alguien ha fijado una recompensa de %ld créditos por la cabeza de %s.", amount , victim->name );
...
void do_addbounty( CHAR_DATA *ch, char *argument )
{
    ...
    if (argument[0] == '\0' )
    {
    	send_to_char( "Uso: recompensa <objetivo> <cantidad>\n\r", ch );
    	return;
    }

    if ( ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "the hunters guild"))
    {
    	send_to_char( "Tu trabajo es ganarte las recompensas, no ofrecerlas.", ch );
    	return;
    }
...
/*    if ( !ch->in_room || ch->in_room->vnum != 6604 )
    ...
    if (argument[0] == '\0' )
     ...
     if ( amount < 5000 )
     {
    	send_to_char( "La recompensa debe ser al menos de 5000 créditos.\n\r", ch );
    	return;
    }
    
    if ( !(victim = get_char_world_ooc( ch, arg )) )
    {
        send_to_char( "Ese jugador no está aquí... tendrás que esperar a que conecte.\n\r", ch );
        return;
    }

    if ( IS_NPC(victim) )
    {
    	send_to_char( "¡Solo puedes ofrecer recompensas por jugadores!\n\r", ch );
	return;
    }
                                        
    if (amount <= 0)
    {
        send_to_char( "¡Buen intento! deberías probar con una recompensa mayor...\n\r", ch );
        return;
    }
    
    if (ch->gold < amount)
    {
    	send_to_char( "¡No posees esa cantidad de créditos!\n\r", ch );
    	return;
    }
...
/* Assassin Experience for Clan Leaders added by Tawnos */
	...
	if (victim->pcdata->clan && (!str_cmp(victim->name, clan->leader) || !str_cmp(victim->name, clan->number1) || !str_cmp(victim->name, clan->number2)))
	{
		...
		ch_printf( ch, "Ganas %ld puntos de experiencia en asesino por la ejecución del líder de un clan.\n\r", exp);
	    }
		...
		gain_exp(ch, exp, ASSASSIN_ABILITY);
		set_char_color( AT_BLOOD, ch);
		ch_printf( ch, "Ganas %ld puntos de experiencia en asesino por la ejecución de un miembro importante de un clan.\n\r", exp);
	    }
        }
      }	    
	...
	if (bounty == NULL)
	{
	    if ( IS_SET(victim->act , PLR_KILLER ) && !IS_NPC(ch) )
	    {
	       ...
	       set_char_color( AT_BLOOD, ch );
	       ch_printf( ch, "Ganas %ld puntos de experiencia en cazarrecompensas por la ejecución de un asesino buscado.\n\r", exp );
	     }
	     else if ( !IS_NPC(ch) ) 
	     {
	        SET_BIT(ch->act, PLR_KILLER );
	        ch_printf( ch, "Ahora eres buscado por el asesinato de %s.\n\r", victim->name );
	     }
	     sprintf( buf, "¡%s ha muerto!", victim->name );
             echo_to_all ( AT_RED , buf, 0 );
	     return;
	...
	set_char_color( AT_BLOOD, ch );
	ch_printf( ch, "Ganas %ld puntos de experiencia en cazarrecompensas y %ld créditos,\n\r por la recompensa sobre %s\n\r", exp, bounty->amount, bounty->target );
	
	sprintf( buf, "¡La recompensa sobre %s ha sido pagada!", victim->name);
	echo_to_all ( AT_RED , buf, 0 );
	sprintf( buf, "¡%s ha muerto!", victim->name );
	echo_to_all ( AT_RED , buf, 0 ); 
