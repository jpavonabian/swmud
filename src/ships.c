...
void do_buymobship(CHAR_DATA *ch, char *argument )
{
  ...
  bool fsys, fplan, fcap;
   ...
   {
   	send_to_char( "Tu organización no te ha concedido permiso para eso.\r\n" ,ch );
   	return;
   }
    ...
    {
        send_to_char("\n\r&z+&W-----------------------------------------------------------------------------&z+\r\n",ch);
        send_to_char("&W|&w                               Naves disponibles                               &W|\r\n",ch);
        send_to_char("&z+&W-----------------------------------------------------------------------------&z+\r\n",ch);

        for(x=0;x<NUM_PROTOTYPES;x++)
		{
		  if(!ch->pcdata->clan && (!str_cmp(ship_prototypes[x].clan, "El Imperio") 
								|| !str_cmp(ship_prototypes[x].clan, "La Nueva República"))) continue;
		  if(ch->pcdata->clan && str_cmp(ship_prototypes[x].clan, ch->pcdata->clan->name) && str_cmp(ship_prototypes[x].clan, "")) continue;
		  if(ch->pcdata->clan && str_cmp(ship_prototypes[x].clan, ch->pcdata->clan->name)) continue;
		  if(ship_prototypes[x].class!= 0 && ship_prototypes[x].class!= 1 && ship_prototypes[x].class!= 2) continue;

            ch_printf(ch,"&W|&w %s%-39.39s&W Tipo:  &w%-13.13s &WCoste:&w %8d &W|\r\n",
            !str_cmp(ship_prototypes[x].clan, "El Imperio") ? "&R" :
            !str_cmp(ship_prototypes[x].clan, "La Nueva República") ? "&G" : "&w",
            ship_prototypes[x].name,
            ship_prototypes[x].class == 0 ? "Fighter" :
            ship_prototypes[x].class == 1 ? "Bombardero" :
            ship_prototypes[x].class == 2 ? "Midtarget" : "Spacecraft",
            ship_prototypes[x].cost);
        }
            
        send_to_char("&z+&W-----------------------------------------------------------------------------&z+\r\n",ch);
        send_to_char("&W|&w               Teclea 'listarnaves' para ver la lista completa               &W|\r\n",ch);
        send_to_char("&z+&W-----------------------------------------------------------------------------&z+\n\r",ch);
      return;
    }
    ...
    {
    	send_to_char("tu organización solo puede comprar sus propios modelos de naves.\n\r", ch);
    	return;
    }
    ...
    if(vnum == -1)
    {
    	send_to_char("Ha habido un problema con tu nave: No hay vnums. Notifícaselo urgentemente a un administrador.\r\n",ch);
    ...
    {
        send_to_char("Tu organización no puede pagar por esta nave.\r\n",ch);
      return;
    }
   clan = ch->pcdata->clan;
   mainclan = ch->pcdata->clan->mainclan ? ch->pcdata->clan->mainclan : clan;
   ...
   {
   	send_to_char( "Tu organización no te ha dado permiso para eso.\r\n" ,ch );
   	return;
   }
   ...
   if(!fsys)
	{
	   send_to_char("No existe este sistema estelar.\n\r", ch);
	   return;
	}
   ...
   if(!fplan)
	{
	   send_to_char("Tu organización no controla ningún planeta en ese sistema estelar.\n\r", ch);
	   return;
	}
   count = 0;
   for(sship = system->first_ship; sship; sship = sship->next_in_starsystem)
	{
	   ...
	   if(sship->class >= SHIP_CRUISER) { fcap = TRUE; caps++; }
	}
   if(!fcap)
	{
    ...
    {
    	send_to_char("Ha habido un problema con tu nave: No se ha podido crear una sala. Notifícaselo urgentemente a los administradores.\r\n",ch);
...
void do_orderclanship(CHAR_DATA *ch, char *argument )
{
  ...
  CLAN_DATA   *mainclan;
   ...
   if ( IS_NPC(ch) || !ch->pcdata )
   {
   	send_to_char( "&R¡Solo los jugadores pueden hacer esto!\r\n" ,ch );
   	return;
   }
   if ( !ch->pcdata->clan )
   {
   	send_to_char( "No perteneces a ninguna organización.\r\n" ,ch );
   	return;
   }
   ...
   {
   	send_to_char( "Tu organización no te ha dado permiso para esto.\r\n" ,ch );
   	return;
   }
    if( !IS_SET(ch->in_room->room_flags2,ROOM_SHIPYARD) )
    {
    	send_to_char("No puedes comprar una nave aquí.\r\n",ch);
      return;
    }
    if(argument == NULL || !argument || argument[0] == '\0')
    {
        ...
        for(x=0;x<NUM_PROTOTYPES;x++)
		{
		  ...
		  if(ch->pcdata->clan && str_cmp(ship_prototypes[x].clan, ch->pcdata->clan->name)) continue;
            ...
            !str_cmp(ship_prototypes[x].clan, "La Nueva República") ? "&G" : "&w",
            ship_prototypes[x].name,
		        ship_prototypes[x].class == SHIP_FIGHTER ? "Caza estelar" :
		        ship_prototypes[x].class == SHIP_BOMBER  ? "Bombardero" : 
		        ship_prototypes[x].class == SHIP_SHUTTLE ? "Lanzadera" : 
		        ship_prototypes[x].class == SHIP_FREIGHTER ? "Carguero" : 
		        ship_prototypes[x].class == SHIP_FRIGATE ? "Fragata" : 
		        ship_prototypes[x].class == SHIP_TT ? "Transporte de tropas" : 
		        ship_prototypes[x].class == SHIP_CORVETTE ? "Corveta" : 
		        ship_prototypes[x].class == SHIP_CRUISER ? "Crucero" : 
			    ship_prototypes[x].class == SHIP_DREADNAUGHT ? "Dreadnaught" : 
		        ship_prototypes[x].class == SHIP_DESTROYER ? "Destructor estelar" : 
		        ship_prototypes[x].class == SHIP_SPACE_STATION ? "Estación espacial" : 
		        ship_prototypes[x].class == LAND_VEHICLE ? "almacén de vehículos" : "Desconocida",
    ...
    {   
        ...
        {
        send_to_char("Ese nombre de nave ya está en uso. Por favor escoge otro.\r\n",ch);
          return;
        }
    }
    if(ch->pcdata->clan->funds < ship_prototypes[ship_type].cost)
    ...
    send_to_char("&wUn tendero introduce datos en su datapad.\r\n",ch);
    send_to_char("&R&CEl vendedor dice:&R&W Todo listo. Gracias.\n\r", ch);
...
void do_ordership(CHAR_DATA *ch, char *argument)
{
  ...
  count=0;
    ...
    {
    	send_to_char("No puedes comprar naves aquí.\r\n",ch);
      return;
    }
   ...
   else
   {
    if(argument == NULL || !argument || argument[0] == '\0')
    {
    ch_printf(ch,"&z+&W------------------------&z El mercado negro &W--------------------------&z+\n\r");
	   ...
	   break;
   	 ...
   	 ch_printf(ch,"&W|&z %-35.35s&W |&z Cantidad: %d&W |&z Coste: %-8d&W |\n\r", bmshipname,marketship->quantity,bmshipcost);
   	 count++;
   	}
   if(count==0)
    ch_printf(ch,"&W|&z                         No hay naves disponibles.                        &W|\n\r");
    ch_printf(ch,"&z+&W--------------------------------------------------------------------&z+\n\r");
    return;
    } 
   }
  ...
  if(!IS_SET(ch->in_room->room_flags2, ROOM_BLACKMARKET))
  {  
   if(!str_cmp(ship_prototypes[ship_type].clan, "El Imperio") ||
      !str_cmp(ship_prototypes[ship_type].clan,"La Nueva República"))
   {
     if(!ch->pcdata->clan)
     {
     ...
     {
      send_to_char("debes encontrar un mercado negro para comprar naves de otros clanes.\n\r", ch);
      return;
     }
   }
  }
    ...
    {   
        if(!str_cmp(ship->name,argument))
        {
        send_to_char("&CEse nombre de nave ya está en uso. Por favor escoge otro.\r\n",ch);
          return;
        }
    }
   if(!IS_SET(ch->in_room->room_flags2, ROOM_BLACKMARKET))
   {
    ...
    {
        send_to_char("No puedes permitirte esa nave.\r\n",ch);
      return;
    }
   }
   else
   {
	...
	if(!str_cmp(argument, "&"))
    {
    	send_to_char("No se admiten colores en el nombre.\n\r", ch);
    	return;
    }
   ...
   if(!IS_SET(ch->in_room->room_flags2, ROOM_BLACKMARKET))
   {
    ...
    send_to_char("Un tendero introduce datos en su datapad.\r\n",ch);
...
{ 
  if(!str_cmp(ship_prototypes[ship_type].sname, marketship->filename)) { 
    ...
    send_to_char("Un contrabandista te sonríe y te muestra tu nave.\n\r", ch);
    send_to_char("&R&CUn contrabandista dice:&R&W Aquí tienes. Es toda tuya.\n\r", ch);
   } 
    ship = make_prototype_ship(ship_type,vnum,ch,argument);
  return;
}
...
while(1)
{
 for(x=0;x<NUM_PROTOTYPES;x++)
 {
  ...
  if(str_cmp(ship_prototypes[x].clan,"El Imperio") && str_cmp(ship_prototypes[x].clan,"La Nueva República"))
   continue;
  if(count == 1)
  {
  ...
  switch( ch->substate )
   {
    ...
    if ( (ship = ship_from_engine(ch->in_room->vnum)) != NULL )
     ...
     if (!ship)
      {
    	send_to_char("Debes estar en la sala de motores de la nave.\n\r", ch);
    	return;
      }
      	...
      	if (obj->item_type == ITEM_MODULE)
	{
     ...
     if ( checkmod == FALSE )
      {
      	send_to_char("¡Necesitas un módulo que instalar!\n\r", ch);
      	return;
      }
...
//Seguro que hay alguna forma de agilizar esto y quitar algún que otro if

  if (modobj->value[0] == AFFECT_ALARM && (ship->alarm == 1))
 {
    send_to_char("La nave ya tiene un sistema de alarmas.\n\r", ch);
    return;
 }
  if (modobj->value[0] == AFFECT_HYPER && (ship->hyperspeed == 1))
 {
    send_to_char("La nave ya tiene un motor hiperespacial primario.\n\r", ch);
    return;
 }
 if (modobj->value[0] == AFFECT_SLAVE && (ship->slave > 0))
 {
    send_to_char("La nave ya tiene un circuito esclavo.\n\r", ch);
    return;
 }
 if (modobj->value[0] == AFFECT_TRACTOR && (ship->tractorbeam > 0))
 {
    send_to_char("La nave ya tiene un rayo tractor.\n\r", ch);
    return;
 }
  ...
  {
    send_to_char("&RNo hay hueco para más.\n\r", ch);
    return;
  }
     ...
     if (i >= maxmod)
      {
	send_to_char("Esta nave ya tiene todos los módulos posibles.\n\r", ch);
	return;
      }

     if( IS_SET(ship->flags, SHIP_SIMULATOR))
      {
	send_to_char("Los simuladores no pueden tener módulos.\n\r", ch);
	return;
      }
     ...
     if ( number_percent( ) < chance )
  	...
  	ch->dest_buf   = str_dup(arg);
       	send_to_char( "&Gcomienzas el largo proceso de instalar un módulo.\n\r", ch);
    	sprintf(buf, "$n usa su caja de herramientas para instalar un módulo en la nave.\n\r");
     	act( AT_PLAIN, buf, ch, NULL, argument , TO_ROOM );
     	add_timer ( ch , TIMER_DO_FUN , 5 , do_installmodule , 1 );
    	return;
      }

      send_to_char("&RNo consigues averiguar como conectarlo correctamente.\n\r",ch);
    ...
    ship = ship_from_engine(ch->in_room->vnum);
    if (!ship){
      send_to_char("Error: algo ha fallado. Notifícaselo urgentemente a la administración.\n\r", ch);
      return;
    }

    if ( number_percent( ) > chance*2 )
     {
       send_to_char("&RTerminas la instalación del módulo y todo parece ir bien...\n\r", ch);
       send_to_char("&R¡El módulo instalado explota!\n\r", ch);
       learn_from_failure( ch, gsn_installmodule );
       return;
     }
	...
	ship->hyperspeed-=mod->ammount; 
      ...
      extract_obj(modobj);
      send_to_char("Finalizas la instalación del nuevo módulo.\n\r", ch);
...
void do_shiplist(CHAR_DATA *ch, char *argument)
{
	int x;
        ...
        send_to_char("\n\r&z+&W-----------------------------------------------------------------------------&z+\r\n",ch);
        send_to_char("&W|&w                             Lista completa de naves                           &W|\r\n",ch);
        send_to_char("&z+&W-----------------------------------------------------------------------------&z+\r\n",ch);
        
        for(x=0;x<NUM_PROTOTYPES;x++)
            ch_printf(ch,"&W|&w  &w(&W%2d&w) %s%-35.35s&W Tipo: %s %-15.15s &W   |\r\n", x+1,
            !str_cmp(ship_prototypes[x].clan, "El Imperio") ? "&R" :
            !str_cmp(ship_prototypes[x].clan, "La Nueva República") ? "&G" : "&w",
            ship_prototypes[x].name,
			!str_cmp(ship_prototypes[x].clan, "El Imperio") ?        "&RImperial" :
			!str_cmp(ship_prototypes[x].clan, "La Nueva República") ?  "&GRepublic" :
																	"&wCivil",
		        ship_prototypes[x].class == SHIP_FIGHTER ? "Caza estelar" :
		        ship_prototypes[x].class == SHIP_BOMBER  ? "Bombardero" : 
		        ship_prototypes[x].class == SHIP_SHUTTLE ? "Lanzadera" : 
        ...
        send_to_char("&z+&W-----------------------------------------------------------------------------&z+\r\n",ch);
        send_to_char("&W|&w               Usa 'mostrarnave (número)' para ver la información de la nave               &W|\r\n",ch);
        send_to_char("&z+&W-----------------------------------------------------------------------------&z+\n\r",ch);
      return;
}
