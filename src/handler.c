...
/* && nifty_is_name( obj->armed_by, xch->name ) Temp removed to see what happens*/ 
	               ...
	               {
    	                   act( AT_WHITE, "¡$p explota en las manos de $n!", obj->carried_by, obj, NULL, TO_ROOM );
	                   act( AT_WHITE, "¡$p explota en tus manos!", obj->carried_by, obj, NULL, TO_CHAR );
	               ...
	               if ( room )
	               {
    	                      if ( !held && room->first_person )
    	                          act( AT_WHITE, "¡$p explota!", room->first_person , obj, NULL, TO_ROOM );
...
void room_explode_1( OBJ_DATA *obj , CHAR_DATA *xch, ROOM_INDEX_DATA *room , int blast )
{
	  ...
	  OBJ_DATA  *robj;
    	  ...
    	  {  
    	                           ...
    	                           act( AT_WHITE, "¡La onda expansiva de una explosión te daña!", rch , obj, NULL, TO_CHAR );
	    		   ...
	    		   {	
	    		        if ( blast > 0 )
	    		        {
	    		            ...
	    		            room_explode_1( obj , xch, pexit->to_room , roomblast );
	    		       	    ...
	    		       	    echo_to_room( AT_WHITE, pexit->to_room , "Oyes una explosión no muy lejos de aquí." );          
    		           }
    		    }
           }
...
bool is_name2( const char *str, char *namelist )
{
   ...
   for( ;; )
   {
      ...
      if( !str_cmp( str, name ) )
         return TRUE;
      if( !str_cmp( str, strip_color(name) ) )
         return TRUE;
   }
}
...
bool is_name2_prefix( const char *str, char *namelist )
{
   ...
   for( ;; )
   {
      ...
      if( !str_prefix( str, name ) )
         return TRUE;
      if( !str_prefix( str, strip_color(name) ) )
         return TRUE;
   }
}
...
/* skill apply types	-Thoric */
	...
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_detrap]> 0)
	  ch->pcdata->learned[gsn_detrap] =
		UMAX( 1, ch->pcdata->learned[gsn_detrap] + mod );
	break;
    case APPLY_DODGE:
	...
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_search]> 0)
	  ch->pcdata->learned[gsn_search] =
		UMAX( 1, ch->pcdata->learned[gsn_search] + mod );
	break;
    case APPLY_DIG:
...
CHAR_DATA *get_char_from_comfreq( CHAR_DATA *ch, char *argument )
{
	...
	return ch;
if ( !str_cmp( argument, ch->comfreq ) || !str_cmp(argument, "me"))
	return ch;
...
/*
 * Return ascii name of an item type in spanish language.
 */
char *item_type_spanish_name( OBJ_DATA *obj )
{
    if ( obj->item_type < 1 || obj->item_type > MAX_ITEM_TYPE )
    {
	bug( "Item_type_name: unknown type %d.", obj->item_type );
