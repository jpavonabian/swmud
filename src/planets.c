...
void do_setplanet( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) )
    {
	send_to_char( "¿Perdón? No he logrado entenderte.\n\r", ch );
	return;
    }
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Uso: setplanet <planeta> <campo> [valor]\n\r", ch );
	send_to_char( "\n\rCampo puede ser:\n\r", ch );
    ...
    if ( !planet )
    {
	send_to_char( "Ese planeta no existe.\n\r", ch );
	return;
    }
	...
	if ( (planet->starsystem = starsystem_from_name(argument)) )
        {
           ...
           send_to_char( "Ok.\n\r", ch );
	       	...
	       	send_to_char( "Ese sistema no existe.\n\r", ch );
	save_planet( planet );
	return;
    }
...
if ( !strcmp( arg2, "import")) 
    ...
    argument = one_argument( argument, arg3 );
       ...
       send_to_char("Ese recurso no existe\r\n", ch); 
       return; 
    } 
...
{   
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Uso: mostrarplaneta <planeta>\n\r", ch );
	return;
    }
    ...
    ch_printf( ch, "&W%s\n\r", planet->name);
    if ( IS_IMMORTAL(ch) )
          ch_printf( ch, "&WNombre de archivo: &G%s\n\r", planet->filename);
    ch_printf( ch, "&WSistema estelar: &G%s\n\r", (planet->starsystem && planet->starsystem->name) ? planet->starsystem->name : "(null)");    
...
void do_planets( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Planeta             Sistema    Gobernado Por                  Soporte Popular\n\r" , ch );
	...
	if(planet->controls != 0) continue;
        ...
        if ( IS_IMMORTAL(ch) && !planet->area )
        {
          ch_printf( ch, "&R¡Advertencia - este planeta no tiene áreas!&G");
          ch_printf( ch, "\n\r" );
        }         
	...
	set_char_color( AT_BLOOD, ch);
        send_to_char( "Aún no hay planetas formados.\n\r", ch );
    }
    send_to_char( "&WEscribe MOSTRARPLANETA para información detallada.\n\r", ch );
...
void do_capture ( CHAR_DATA *ch , char *argument )
{
   ...
   if ( !ch->pcdata->clan )
   {
       send_to_char ( "¡Necesitas ser miembro de una organización para hacer eso!\n\r" , ch );
       return;
   }
   ...
   {
       send_to_char ( "Debes estar en un planeta para capturarlo.\n\r" , ch );
       return;
   }   
   
   if ( clan == planet->governed_by )
   {
       send_to_char ( "Tu organización ya controla este planeta.\n\r" , ch );
       return;
   }

   if ( clan->clan_type == CLAN_CRIME || clan->clan_type == CLAN_GUILD )
   {
        send_to_char( "¡Tu clan no puede capturar planetas!\n\r", ch);
        return;
   }
   ...
   if ( planet->starsystem )
   {
       ...
       for ( ship = planet->starsystem->first_ship ; ship ; ship = ship->next_in_starsystem )
       {
          ...
          {
             send_to_char ( "Un planeta no puede ser capturado mientras tenga una nave orbitando que lo proteja.\n\r" , ch );
             return;
          }
       }
   }
   ...
   {
       send_to_char ( "Este planeta está protegido por soldados.\n\r" , ch );
       send_to_char ( "Tendrás que eliminar a todas las fuerzas enemigas antes de capturarlo.\n\r" , ch );
       return;
   }
  if(planet->governed_by->name && planet->governed_by->name != NULL)
   ...
   if(!str_cmp(planet->governed_by->name, "Neutral"))
	{
	   send_to_char("Este planeta no puede ser capturado.\n\r", ch);
	   return;
	}
	}
   if ( planet->pop_support > 0 )
   {
       send_to_char ( "Los ciudadanos no están a favor de un cambio de líder ahora mismo.\n\r" , ch );
       return;
   }
   ...
   if ( support < 0 )
   {
       send_to_char ( "¡Tu organización no tiene suficiente soporte popular!\n\rIntenta mejorar la lealtad de los planetas que ya controlas.\n\r" , ch );
       return;
   }
   ...
   sprintf( buf , "¡%s ha sido capturado por %s!", planet->name, clan->name );   
...
void do_imports( CHAR_DATA *ch, char *argument )
{
   ...
   if (argument[0] == '\0')
   {
      send_to_char("Uso: importes <planeta>\r\n", ch);
      return;
   }
   ...
   if (!planet)
   {
      send_to_char("&REse planeta no existe\r\n", ch);
      return;
   }
   ch_printf(ch,"&BDatos de importación y exportación de %s:\r\n", planet->name);
   ch_printf(ch,"&GRecursos    &CImporta     &YExporta    &PProduce    &RConsume         &GCantidad\r\n");
   ch_printf(ch, "&G----------   ------     ------    --------    --------          ------\r\n");
   for (i = 1; i < CARGO_MAX; i++)
