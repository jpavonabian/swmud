...
void do_induct( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "¿Reclutar a quién?\n\r", ch );
	return;
    }

    if ( ( victim = get_char_room( ch, arg ) ) == NULL )
    {
	send_to_char( "Ese jugador no está aquí.\n\r", ch);
	return;
    }
	...
	if ( victim->pcdata->clan == clan )
	  ...
	  send_to_char( "¡Ese jugador ya pertenece a una organización!\n\r", ch );
	return;
      }
	...
	if ( victim->pcdata->clan == clan )
	  ...
	  send_to_char( "Ese jugador ya pertenece a tu organización!\n\r", ch );
	return;
      }
      else
      {
...
void do_gatherclans( CHAR_DATA *ch, char *argument )
{
    ...
    for ( clan = first_clan; clan; clan = clan->next )
    {
        ...
        ch_printf( ch, "&WOrganización: &Y%s\n\r",
                   clan->name);
        ch_printf( ch, "  &WPlanetas: &O%-2d       &WSoporte Popular: &O%-3d&W    &WIngresos: &O%-10ld\n\r",
                   pCount,support,revenue);
        ch_printf( ch, "  &WDivisión             Líderes             Naves/Vehículos/Miembros/Tropas/Fondos\n\r");
    ...
    ch_printf( ch, "&WGrupos autónomos      Líderes             Naves/Vehículos/Miembros/Fondos\n\r");
    for ( clan = first_clan; clan; clan = clan->next )
    {
...
void do_shove( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("Intentas empujarle pero rebotas y caes al suelo. Qué coño...\n\r", ch);
    ...
    {
	act( AT_PLAIN, "$N no está de pie.", ch, NULL, victim, TO_CHAR );
	return;
    }
	...
	if ( arg2[0] == '\0' )
    {
	send_to_char( "¿Empujarle en que dirección?\n\r", ch);
	return;
    }
    else  if ( ( dir = get_door( arg2 ) ) != -1 )
    {
      ...
      {
         send_to_char( "¿Empujarle contra una pared? ¡Cabrón!\n\r", ch );
         return;
      }
      ...
      if ( IS_SET( pexit->exit_info, EX_CLOSED ) )
      {
       ...
       to_room = pexit->to_room;
		...
		{
		  send_to_char("Ese jugador no puede entrar en ese área.\n\r", ch);
	...
	return;
     ...
     {
      act( AT_PLAIN, "Eso no es una dirección o una nave.", ch, NULL, argument, TO_CHAR );
      return;
     }
     else
     {
...
void do_drag( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim == ch )
    {
	send_to_char("Te agarras a ti mismo del cuello e intentas arrastrarte, pero no vas a ningún lugar.\n\r", ch);
	return;
    }
    ...
    if ( arg2[0] == '\0' )
    {
	send_to_char( "¿Arrastrarle en qué dirección?\n\r", ch);
	return;
    }
...
if (chance < number_percent( ))
{
  ...
  return;
	...
	act( AT_ACTION, "Arrastras a $M a la siguiente habitación.", ch, NULL, victim, TO_CHAR );
    ...
    send_to_char("No puedes arrastrarle si está de pie.\n\r", ch);
    return;
}
...
void do_enlist( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_NPC(ch) || !ch->pcdata )
	{
        ...
        if ( ch->pcdata->clan )
        {
            ch_printf( ch , "Debes dimitir de %s antes de alistarte en una nueva organización.\n\r", ch->pcdata->clan->name );
            return;
        }

        if ( IS_SET( ch->in_room->room_flags , ROOM_R_RECRUIT ) )
           clan = get_clan( "La Nueva República" );
        ...
        if ( !clan )
        {
            send_to_char( "Ese clan no está reclutando nuevos miembros ahora mismo.\n\r", ch );
	    return;
        }
...
/*        SET_BIT( ch->speaks, LANG_CLAN );*/
	...
	ch_printf( ch, "Bienvenido a %s.\n\r", clan->name );
        send_to_char("Para ajustar tu salario, habla con el líder del clan.\n\r", ch);
...
void do_resign( CHAR_DATA *ch, char *argument )
{
        ...
        if ( IS_NPC(ch) || !ch->pcdata )
	{
        ...
        if ( clan == NULL )
        {
	    send_to_char( "Para dimitir de una organización, primero debes formar parte de alguna.\n\r", ch );
	    return;
	}

       if ( !str_cmp( ch->name, ch->pcdata->clan->leader ) )
       {
           ch_printf( ch, "No puedes dimitir de %s... ¡eres el líder!\n\r", clan->name );
           return;
       }
...
void do_clan_withdraw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) || !ch->pcdata->clan )
    {
	send_to_char( "No parece que pertenezcas a ninguna organización de la que retirar fondos...\n\r", ch );
	return;
    }
    ...
    if ( !amount )
    {
	send_to_char( "¿Cuánto dinero quieres retirar?\n\r", ch );
	return;
    }
    ...
    ch_printf( ch,  "Retiras %ld créditos de los fondos de %s.\n\r", amount, clan->name );
...
void do_clan_donate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) || !ch->pcdata->clan )
    {
	send_to_char( "Eres muy generoso, pero no tienes organización...\n\r", ch );
	return;
    }
    ...
    if ( !amount )
    {
	send_to_char( "¿Cuánto dinero quieres donar?\n\r", ch );
	return;
    }
    ...
    ch_printf( ch,  "Donas %ld créditos a los fondos de %s.\n\r", amount, clan->name );
...
void do_appoint ( CHAR_DATA *ch , char *argument )
{
    ...
    if (  str_cmp( ch->name, ch->pcdata->clan->leader  )  )
    {
	send_to_char( "¡Solo tu líder puede hacer eso!\n\r", ch );
	return;
    }
...
void do_demote ( CHAR_DATA *ch , char *argument )
{
    ...
    {
	send_to_char( "No puedes degradarle más.\n\r", ch );
	return;
    }
    save_clan ( ch->pcdata->clan );
...
void do_war ( CHAR_DATA *ch , char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "¿Declarar la guerra a quién?\n\r", ch );
	return;
    }
...
void do_empower ( CHAR_DATA *ch , char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "¿Permitir a quién hacer qué?\n\r", ch );
	return;
    }
    ...
    else if ( !str_cmp( arg2, "buscado" ) )
   {
      ...
      DISPOSE( victim->pcdata->bestowments );
      victim->pcdata->bestowments = str_dup( buf );
      ch_printf( victim, "%s te ha dado permiso para decidir a quién se busca por asesinato.\n\r", ch->name);
      send_to_char( "Ok, permiso para decidir quién es buscado por asesinato concedido.\n\r", ch);
   }
    else
    {
      ...
      send_to_char(	"buscado:      poder para decidir quién se busca y quién se deja de buscar por asesinato en los planetas de tu clan\n\r", ch);
...
void do_outlaw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "¿Buscar por asesinato a quién?\n\r", ch );
	return;
    }
    ...
    send_to_char("Ese jugador ahora será buscado por asesinato en todos los planetas de tu clan.\n\r", ch);
    ch_printf(victim,"&W&RUn leve pitido suena en tu comunicador y oyes una voz que dice, 'Atención todos los ciudadanos, %s ahora es buscado por asesinato en los planetas de %s hasta nuevo aviso. Los guardias le atacarán en cuanto le vean. Gracias.'\n\r", victim->name, clan->name);
...
void do_unoutlaw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "¿Dejar de buscar a quién?\n\r", ch );
	return;
    }
    ...
    ch_printf(victim,"&W&GUn leve pitido suena en tu comunicador y oyes una voz que dice, 'Atención todos los ciudadanos, %s ha sido absuelto de todos los crímenes cometidos en los planetas de %s. Ya no será buscado. Gracias.'\n\r", victim->name, clan->name);
...
void do_clanstat( CHAR_DATA *ch, char *argument)
{
   ...
   if(IS_NPC(ch))
    {
	send_to_char("¡Eres un mob, no necesitas esa información!\n\r", ch);
	return;
    }

   if(!ch->pcdata->clan)
    {
	send_to_char("¡Ni siquiera estás EN un clan!\n\r", ch);
	return;
    }
   ...
   if( clan->leader[0] != 0 )
    ch_printf(ch, "&x&W|       &BLíder&W: %-12.12s           &BTropas&W: %-4.4d       |\n\r", clan->leader, clan->troops );
   else
    ch_printf(ch, "&x&W|       &BLíder&W: Nadie.              &BTropas&W: %-4.4d           |\n\r", clan->troops );
    ...
    for ( subclan = clan->first_subclan ; subclan ; subclan = subclan->next_subclan )
     {
      ...
      if( clan->leader[0] != 0 )
       ch_printf(ch, "&x&W|       &BLíder&W: %-12.12s       &BNaves Espaciales&W: %-2.2s         |\n\r", subclan->leader, subclan->spacecraft );
      else
       ch_printf(ch, "&x&W|       &BLíder&W: Nadie.              &BTropas&W: %-2.2s             |\n\r", subclan->leader, subclan->troops );
...
void do_setwage( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) || !ch->pcdata->clan )
    {
	send_to_char( "¡No eres el líder de un clan!\n\r", ch );
	return;
    }
  ...
  send_to_char( "¡Solo retira tu salario de los fondos del clan!\n\r", ch);     
    ...
    if ( !str_cmp( victim->name, clan->leader ))
    {
        send_to_char( "El líder tiene su propio salario.\n\r",ch);
	return;
    }

    if ( clan->number2 == ch->name && clan->number1 == victim->name )
    {
	send_to_char( "No puedes ajustar el salario de alguien con más rango que tu.\n\r", ch);
	return;
    }
    ...
    ch_printf(ch, "&w&GSalario de %s fijado en %d créditos por día.\n\r", victim->name, amount);
    ch_printf(victim, "&w&GTu salario ha sido fijado en %d créditos por día.\n\r", amount);
...
void do_clans( CHAR_DATA *ch, char *argument )
{
    ...
    for ( clan = first_clan; clan; clan = clan->next )
    {
        ...
        ch_printf( ch, "&z| &WOrganización: &w%-25.25s &WTipo: &w%-15.15s &z|\n\r",
    ...
    {
	set_char_color( AT_BLOOD, ch);
        send_to_char( "Aún no hay organizaciones formadas.\n\r", ch );
    }
...
void do_members(CHAR_DATA *ch, char *argument)
{
 ...
 {
   send_to_char("&RTu clan no tiene una descripción corta. Reporta esto a un inmortal.\n\r", ch);
   return;
 }
 ...
 if ( ( fpList = fopen(list, "r" ) ) == NULL )
 {
    ...
    return;
     ...
     sprintf(color, "%s", !str_cmp(clan->name, "El Imperio") ? "&R" : !str_cmp(clan->name, "La Nueva República") ? "&B" : "&G");
