...
CHAR_DATA *find_keeper( CHAR_DATA *ch )
{
    ...
    {
	do_say( keeper, "Por favor, vuelve más tarde." );
	return NULL;
    }

    if ( time_info.hour > pShop->close_hour )
    {
	do_say( keeper, "Por favor, vuelve mañana." );
	return NULL;
    }
    
    if ( !knows_language( keeper, ch->speaking, ch ) )
    {
	do_say( keeper, "No puedo entenderte." );
	return NULL;
    }
...
void do_buy( CHAR_DATA *ch, char *argument )
{
   ...
   else if ( !str_cmp( "atm", argument ) || !str_cmp( "debit", argument ) )
   {
     ...
     {
       send_to_char( "¡No tienes tu tarjeta!\n\r", ch );
       return;    
     }    
    }
	...
	if ( !pRoomIndexNext )
	{
	    ...
	    send_to_char( "Lo siento, no puedes comprar eso aquí.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "No puedes permitírte eso.\n\r", ch );
	    return;
	}
	else if ( (ch->pcdata->bank < 10 * pet->top_level * pet->top_level) && debit == TRUE )
	{
	  send_to_char( "No tienes créditos suficientes en tu cuenta bancaria.\n\r", ch );
          return;    
        } 
	...
	sprintf( buf, "%sUn collar en el que se lee 'Pertenezco a %s'.\n\r",
	...
	send_to_char( "Disfrute su mascota.\n\r", ch );
    	act( AT_ACTION, "$n compra $N como mascota.", ch, NULL, pet, TO_ROOM );
	return;
    }
    else
    {
	...
	if ( is_number( arg ) )
	{
	    ...
	    if ( noi > mnoi )
	    {
		act( AT_TELL, "$n te dice 'No vendo esos objetos"
    ...
    else if ( !str_cmp( "atm", argument ) || !str_cmp( "debit", argument ) )
    {
      ...
      {
        send_to_char( "¡no tienes tu tarjeta de crédito!\n\r", ch );
        return;    
      }    
     }  
	...
	if( !IS_NPC(ch) && ch->pcdata->learned[gsn_bargain] > 0 && ch->pcdata->learned[gsn_bargain] > number_percent())
	 {
	   ch_printf(ch,"¡Consigues negociar un descuento de %d ccréditos a %d créditos!\n\r", cost, (cost/3)+(cost/2));
	   cost = (cost/3) + (cost/2);
	...
	{
	    act( AT_TELL, "$n te dice 'No vendo eso. Prueba a 'listar' la mercancía.'",
	...
	{
	    interpret( keeper, ", sonríe" );
	    act( AT_TELL, "$n te dice 'No tengo suficientes en stock"
	     " para vender más de uno a la vez.'", keeper, NULL, ch, TO_VICT );
	    ch->reply = keeper;
	    return;
	}
	...
	{
	    act( AT_TELL, "$n te dice 'No puedes permitirte comprar $p.'",
	...
	{
	    send_to_char( "¡Cuando estás a punto de deslizar la tarjeta recuerdas que no tienes suficiente dinero!\n\r", ch );
	    return;	    
        }  
	...
	{
	    act( AT_TELL, "$n te dice '¡Solo es un prototipo! No puedo vender eso...'", 
		keeper, NULL, ch, TO_VICT );
      	    ch->reply = keeper;
	    return;
	}
...
void do_list( CHAR_DATA *ch, char *argument )
{
	...
	if ( !found )
	    send_to_char( "Lo siento, no tenemos mascotas a la venta ahora mismo.\n\r", ch );
	return;
    }
    else
    {
	...
	for ( obj = keeper->last_carrying; obj; obj = obj->prev_content )
	{
		...
		{
		    found = TRUE;
		    ch_printf( ch, "%s[Coste] {ref} Objeto\n\r", color_str( AT_LIST, ch) );
		}
		ch_printf( ch, "%s[%5d] {%3d} %s%s%s.\n\r",
		    color_str(AT_LIST, ch), cost, oref, capitalize( obj->short_descr ), color_str(AT_LIST, ch),
		    IS_SET(obj->extra_flags, ITEM_HUTT_SIZE) ? " (Tamaño jutt)" :
		    ( IS_SET(obj->extra_flags, ITEM_LARGE_SIZE) ? " (grande)" :
		    ( IS_SET(obj->extra_flags, ITEM_HUMAN_SIZE) ? " (medio)" :
		    ( IS_SET(obj->extra_flags, ITEM_SMALL_SIZE) ? " (pequeño)" :
		    "" ) ) ) );
	       }
	    }
	}
	...
	{
	    if ( arg[0] == '\0' )
		send_to_char( "No puedes comprar nada aquí.\n\r", ch );
	    else
		send_to_char( "No puedes comprar eso aquí.\n\r", ch );
	}
	return;
    }
}
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( AT_TELL, "$n te dice 'No tienes ese objeto.'", keeper, NULL, ch, TO_VICT );
	ch->reply = keeper;
	return;
    }
    ...
    {
        send_to_char("¿Qué tratas de venderme? ¡No compro aire!\n\r", ch );
        return;
    }
    ...
    {
	send_to_char( "¡No puedes soltar eso ir!\n\r", ch );
	return;
    }

    if ( obj->timer > 0 )
    {
	act( AT_TELL, "$n te dice, '$p se desvalora rápidamente...'", keeper, obj, ch, TO_VICT );
	return;
    }

    if ( ( cost = get_cost( ch, keeper, obj, FALSE ) ) <= 0 )
    {
	act( AT_ACTION, "$n mira desinteresadamente in $p.", keeper, obj, ch, TO_VICT );
	return;
    }

    if ( cost > keeper->gold )
    {
	act( AT_TELL, "$n hace una transacción de créditos.", keeper, obj, ch, TO_VICT );
        lower_economy( ch->in_room->area, cost-keeper->gold );
    }
    
    separate_obj( obj );
    act( AT_ACTION, "$n vende $p.", ch, obj, NULL, TO_ROOM );
    sprintf( buf, "Vendes $p por %d crédito%s.",
	cost, cost == 1 ? "" : "s" );
    ...
    else  if ( IS_SET( obj->extra_flags , ITEM_CONTRABAND) )
   {
       ...
       ch_printf( ch, "Recibbes %ld puntos de experiencia en contrabando por descargar la mercancía ilegal.\n\r " , ch_exp );
...
void do_value( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( AT_TELL, "$n te dice 'No tienes ese objeto.'",
		keeper, NULL, ch, TO_VICT );
	ch->reply = keeper;
	return;
    }
    ...
    if ( !can_drop_obj( ch, obj ) )
    {
	send_to_char( "¡No puedes soltar eso!\n\r", ch );
	return;
    }

    if ( ( cost = get_cost( ch, keeper, obj, FALSE ) ) <= 0 )
    {
	act( AT_ACTION, "$n mira desinteresadamente $p.", keeper, obj, ch, TO_VICT );
	return;
    }

    sprintf( buf, "$n te dice 'Te daré %d créditos por $p.'", cost );
                 ...
                 char *arg, int maxgold, char *fixstr, char*fixstr2 )
{
   ...
   {
       if (cost != -2)
       act( AT_TELL, "$n te dice, 'Lo siento, no puedo hacer nada con $p.'", 
            keeper, obj, ch, TO_VICT );
       else
	  act( AT_TELL, "¡$n te dice, '¡$p me suena familiar!'", keeper, obj, ch, TO_VICT );
   }
               /* "repair all" gets a 10% surcharge - Gorog */

   else if ( (cost = strcmp("todo",arg) ? cost : 11*cost/10) > ch->gold )
   {
      sprintf( buf,
       "$N te dice, 'Le costará %d crédito%s %s %s...'", cost,
        cost == 1 ? "" : "s", fixstr, obj->name );
      act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
      act( AT_TELL, "$N te dice, 'Por lo que veo no se lo puede permitir.'", ch,
              NULL, keeper, TO_CHAR );
   }
   else
   {
      sprintf( buf, "$n da $p a $N, que rápidamente %s.", fixstr2 );
      act( AT_ACTION, buf, ch, obj, keeper, TO_ROOM );
      sprintf( buf, "$N te carga %d crédito%s por %s $p.",
      ...
      if ( keeper->gold > maxgold )
      {
          boost_economy( keeper->in_room->area, keeper->gold - maxgold/2 );
          keeper->gold = maxgold/2;
          act( AT_ACTION, "$n pone algunos créditos dentro de una gran bolsa.", keeper, 
		NULL, NULL, TO_ROOM );
      }
      ...
      {
          default:
            send_to_char( "Por alguna razón piensas que te han estafado...\n\r", ch);
            break;
          case ITEM_ARMOR:
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
    ...
    for ( obj = ch->first_carrying; obj != NULL ; obj = obj->next_content )
    {
        ...
        ||   obj->item_type == ITEM_DEVICE ) ){
            ...
            else if ( ( cost = get_repaircost( keeper, obj ) ) < 0 )
            {
               if (cost != -2)
               act( AT_TELL,
                    "$n te dice, 'Lo siento, no puedo hacer nada con $p.'",
                    keeper, obj, ch, TO_VICT );
               else
               act( AT_TELL, "$n te dice, '¡$p me suena familiar!'",
                    keeper, obj, ch, TO_VICT );
            }
            else 
            {
            sprintf( buf,
            "$N te dice, 'Te costará %d crédito%s %s %s'",
    ...
    if ( total > 0 )
    {
       send_to_char ("\n\r", ch);
       sprintf( buf,
          "$N te dice, 'Te costará %d crédito%s en total.'",
          total, cost == 1 ? "" : "s" );
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
       strcpy( pbuf,
       "$N te dice, 'Recuerda que te cobraré un 10% extra por recargar y reparar todo.'");
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    }
}
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    {
      if (cost != -2)
	act( AT_TELL, "$n te dice, 'Lo siento, no puedo hacer nada con $p.'", keeper, obj, ch, TO_VICT );
      else
	act( AT_TELL, "$n te dice, '¡$p me suena familiar!'", keeper, obj, ch, TO_VICT );
      return;
    }
    ...
    sprintf( buf,
       "$N te dice, 'Te costará %d crédito%s %s eso...'", cost,
       cost == 1 ? "" : "s", fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    if ( cost > ch->gold )
      act( AT_TELL, "$N te dice, 'Por lo que veo no te lo puedes permitir.'", ch,
	 NULL, keeper, TO_CHAR );
