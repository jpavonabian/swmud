...
bool check_pos( CHAR_DATA *ch, sh_int position )
{
	...
	{
	case POS_DEAD:
	    send_to_char( "Es un poco complejo estando muerto...\n\r", ch );
	    break;

	case POS_MORTAL:
	case POS_INCAP:
	    send_to_char( "Tus heridas no te lo permiten.\n\r", ch );
	    break;
	...
	case POS_RESTING:
	    send_to_char( "Nah... Estás tan bien así...\n\r", ch);
	    break;
	...
	case POS_FIGHTING:
	    send_to_char( "¡De ninguna manera! ¡Estás luchando!\n\r", ch);
	    break;
...
*/
    ...
    if ( ( !IS_NPC(ch) && IS_SET(ch->act, PLR_SLOG) ))
    {
        /* Added by Narn to show who is switched into a mob that executes
           a logged command.  Check for descriptor in case force is used. */
        if ( ch->desc && ch->desc->original )
          sprintf( log_buf, "SecretLog %s (%s): %s", ch->name,
                   ch->desc->original->name, logline );
        else
