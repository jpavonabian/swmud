...
void learn_from_success( CHAR_DATA *ch, int sn )
{
	...
	if ( ch->pcdata->learned[sn] == 100 )	 /* fully learned! */
	{
	    ...
	    ch_printf( ch, "¡Dominas %s a la perfección! Recibes %d puntos de experiencia!\n\r",
...
void do_gouge( CHAR_DATA *ch, char *argument )
{
	...
	{
		act( AT_BLOOD, "¡Tu víctima tiene tantas heridas que solo basta un puñado de tierra para matarle!",
...
void do_dig( CHAR_DATA *ch, char *argument )
{
	...
	default:
	  ...
	  if ( arg[0] != '\0' )
	  {
	      ...
	      &&     get_dir(arg) == -1 )
	      {
		  send_to_char( "¿Qué dirección es esa?\n\r", ch );
		  return;
	      }
	      if ( pexit )
	      {
	  ...
	  else
	  {
		  ...
		  case SECT_UNDERWATER:
		    send_to_char( "No puedes cavar aquí.\n\r", ch );
		    return;
		  case SECT_AIR:
...
void do_search( CHAR_DATA *ch, char *argument )
{
	...
	case 1:
	    if ( !ch->dest_buf )
	    {
		send_to_char( "¡Tu búsqueda ha sido interrumpida!\n\r", ch );
	...
	case SUB_TIMER_DO_ABORT:
	    DISPOSE( ch->dest_buf );
	    ch->substate = SUB_NONE;
	    send_to_char( "Detienes tu búsqueda...\n\r", ch );
	    return;
    }
    ch->substate = SUB_NONE;
    if ( arg[0] == '\0' )
	...
	{
	    act( AT_SKILL, "¡Tu búsqueda rebela $d!", ch, NULL, pexit->keyword, TO_CHAR );
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "¿Robar qué a quién?\n\r", ch );
	return;
    }
	...
	ch->gold     += amount;
	victim->gold -= amount;
	ch_printf( ch, "¡Ah!  Le robas %d créditos.\n\r", amount );
        sprintf(logbuf, "%s robó %d créditos a %s.\n\r", ch->name, amount, IS_NPC(victim) ? victim->short_descr : victim->name);
        log_string(logbuf);
...
void do_dualstab( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "¿ataquedoble a quién?\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Debes empuñar dos armas punzantes.\n\r", ch );
	return;
    }

    if ( victim->fighting )
    {
	send_to_char( "No puedes, está luchando.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Deberías esconderte primero.\n\r", ch);
	return;
    }
...
void do_backstab( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Debes empuñar un arma que permita apuñalar.\n\r", ch );
	return;
    }

    if ( victim->fighting )
    {
	send_to_char( "No puedes apuñalar a alguien que está en combate.\n\r", ch );
	return;
    }
...
void do_rescue( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Pero si no está luchando...\n\r", ch );
	return;
    }
...
void do_bash( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char(
	    "No tienes idea de cómo hacer eso.\n\r", ch );
	return;
    }
...
void do_disarm( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Debes empuñar un arma para desarmar.\n\r", ch );
	return;
    }
    ...
    if ( ( obj = get_eq_char( victim, WEAR_WIELD ) ) == NULL )
    {
	send_to_char( "Tu oponente no está empuñando ningún arma.\n\r", ch );
	return;
    }
...
void do_pick( CHAR_DATA *ch, char *argument )
{
	...
	{
	act( AT_PLAIN, "$N está demasiado cerca de la cerradura.",
		ch, NULL, gch, TO_CHAR );
	    return;
	}
    }
...
//        	echo_to_all ( AT_RED , buf, 0 );	
              if (ship->alarm == 1)
              {
                 if((victim = get_char_world_ooc(ch, ship->owner)) != NULL)
                   ch_printf(victim, "&RRecibes una señal del módulo de alarma instalado en %s.&W", ship->name);
              }                
...
void do_hide( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->mount )
    {
	send_to_char( "No puedes hacer eso mientras estés montando.\n\r", ch );
	return;
    }
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( ch ) < LEVEL_IMMORTAL )
    {
	   ...
	   return;
        ...
        {
	   send_to_char( "solo puedes hacer recall a tu área asignada.\n\r", ch );
	   return;
        }   
    }
...
void do_aid( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->hit <= -400 )
    {
	act( AT_PLAIN, "La condición de $N está más allá de tus capacidades.", ch,
	     NULL, victim, TO_CHAR);
	return;
    }
...
void do_poison_weapon( CHAR_DATA *ch, char *argument )
{
  ...
  &&  ch->pcdata->learned[gsn_poison_weapon] <= 0  )
    {
    send_to_char( "¿Qué te crees, un asesino?\n\r", ch );
    return;
    }
  ...
  one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "¿Qué intentas envenenar?\n\r",    ch );
	return;
    }
    if ( ch->fighting )
    {
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !ch->fighting )
  {
    send_to_char( "¡Pero si no estás luchando!\n\r", ch );
    return;
  }
  
  if ( IS_AFFECTED(ch, AFF_BERSERK) )
  {
    send_to_char( "¡No puedes acumular más furia!\n\r", ch );
    return;
  }
...
void do_hitall( CHAR_DATA *ch, char *argument )
{
  ...
  {
    send_to_char( "¡Aquí no hay nadie!\n\r", ch );
    return;
  }
...
void do_scan( CHAR_DATA *ch, char *argument )
{
  ...
  if ( argument[0] == '\0' )
  {
    send_to_char( "Otear en una dirección...\n\r", ch );
    return;
  }

  if ( ( dir = get_door( argument ) ) == -1 )
  {
    send_to_char( "¿Otear en qué dirección?\n\r", ch );
    return;
  }
...
*/
  ...
  for ( dist = 1; dist <= max_dist; )
  {
	...
	act( AT_GREY, "Tu visión hacia $t es bloqueada por una pared.", ch,
	  dir_name2[dir], NULL, TO_CHAR );
       else
        act( AT_GREY, "Tu visión hacia $t es bloqueada por una puerta.", ch, 
	  dir_name2[dir], NULL, TO_CHAR );
      break;
    }
    ...
    {
      act( AT_GREY, "Tu visión hacia $t es bloqueada por una sala pribada.", ch, 
	dir_name2[dir], NULL, TO_CHAR );
      break;
    }
    char_from_room( ch );
