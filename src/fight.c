...
/*    if ( victim->race == RACE_DUINUOGWUIN )
	...
	if(ch->force_type == FORCE_JEDI && wield && wield->value[3] == WEAPON_DUAL_LIGHTSABER)
	{
		...
		{
			send_to_char("No estás entrenado en el uso de sables de luz de doble hoja, pierdes tu tiempo.\n\r", ch);
			act(AT_YELLOW, "$n valancéa torpemente su sable de luz de doble hoja.", ch, NULL, NULL, TO_ROOM);
			return rNONE;
		}
	}
    ...
    /* 
     ...
     if ( dt == (TYPE_HIT + WEAPON_BLASTER ) && wield && wield->item_type == ITEM_WEAPON )
     {
     	if ( wield->value[4] < 1  )
     	{ 
            act( AT_YELLOW, "$n te dispara con su arma láser pero no ocurre nada.",  ch, NULL, victim, TO_VICT    );
            act( AT_YELLOW, "*CLICK*... ¡Tu arma láser necesita munición!", ch, NULL, victim, TO_CHAR    );            
            if ( IS_NPC(ch) )
     	    {
		...
		act( AT_BLUE, "¡Anillos azules de energía del arma láser de $N te golpean y aturden!", victim, NULL, ch, TO_CHAR );
		act( AT_BLUE, "¡Anillos azules de energía  de tu arma láser golpean a $N, aturdiéndole!", ch, NULL, victim, TO_CHAR );
		act( AT_BLUE, "¡Anillos azules de energía del arma láser de $n golpean a $N, aturdiéndole!", ch, NULL, victim, TO_NOTVICT );
		...
		if ( !IS_AFFECTED( victim, AFF_PARALYSIS ) )
		{
            ...
            {
               act( AT_BLUE, "¡Anillos azules de energía del arma láser de $N te golpean pero logras resistir!", victim, NULL, ch, TO_CHAR );
		act( AT_BLUE, "¡Anillos azules de energía  de tu arma láser golpean a $N, pero logra resistir!", ch, NULL, victim, TO_CHAR );
		act( AT_BLUE, "¡Anillos azules de energía del arma láser de $ngolpean a $N, pero logra resistir!", ch, NULL, victim, TO_NOTVICT );
     ...
     else if ( dt == (TYPE_HIT + WEAPON_BOWCASTER ) && wield && wield->item_type == ITEM_WEAPON )
     {
     	if ( wield->value[4] < 1  )
     	{ 
            act( AT_YELLOW, "$n te dispara con su arco de energía pero no pasa nada.",  ch, NULL, victim, TO_VICT    );
            act( AT_YELLOW, "¡*CLICK*... tu arco de energía requiere un nuevo cartucho!", ch, NULL, victim, TO_CHAR    );            
            if ( IS_NPC(ch) )
     	    {
     	        do_remove( ch, wield->name );
     	    }
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	    ...
	    if ( check_parry( ch, victim ) )
	        ...
	        if((victwield = get_eq_char( victim, WEAR_WIELD)) != NULL)
		 {
	         if((victwield->value[3] == WEAPON_LIGHTSABER || victwield->value[3] == WEAPON_DUAL_LIGHTSABER) && wield->value[3] == WEAPON_BLASTER)
                  {
                   act( AT_WHITE, "¡Predices mediante la fuerza el disparo de arma láser de $n y colocas la hoja de tu sable de luz en la posición adecuada, interceptando y reflejando su rayo!", ch, NULL, victim, TO_VICT );
                   act( AT_WHITE, "¡$N intercepta el disparo de tu arma láser con la hoja de su sable de luz y te lo refleja!", ch, NULL, victim, TO_CHAR );
		   act( AT_WHITE, "¡$N intercepta con la hoja de su sable de luz el disparo de arma láser de $N y se lo refleja!", ch, NULL, victim, TO_NOTVICT);
    ...
    */
    ...
    {
    case POS_MORTAL:
	act( AT_DYING, "$n tiene heridas de muerte y morirá lentamente si nadie le ayuda.",
	    victim, NULL, NULL, TO_ROOM );
	send_to_char( "&RTienes heridas mortales y morirás lentamente si nadie te ayuda.",victim);
	break;

    case POS_INCAP:
	act( AT_DYING, "$n es incapaz de reaccionar y morirá lentamente si nadie le ayuda.",
	    victim, NULL, NULL, TO_ROOM );
	send_to_char( "&RNo eres capaz de reaccionar y morirás lentamente si nadie te ayuda.",victim);
	break;

    case POS_STUNNED:
        if ( !IS_AFFECTED( victim, AFF_PARALYSIS ) )
        {
	  act( AT_ACTION, "A $n le han aturdido, pero se recuperará.",
	    victim, NULL, NULL, TO_ROOM );
	  send_to_char( "&RTe han aturdido, pero te recuperarás.",victim);
	}
	break;
	...
	{
	   act( AT_DANGER, "¡Desearías que tus heridas no sangraran tanto!",
		victim, 0, 0, TO_CHAR );
	   if ( number_bits(2) == 0 )
		worsen_mental_state( ch, 1 );
	}
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
	     xp = 0;
	     sprintf( buf, "No recibes experiencia por matar tropas de tu organización.\n\r");
...
// Función para puentear bug. hay que externalizarla o cambiarla por la original.
...
/*
 * Función String Copy, para copiar un texto dentro de un buffer.
 * Si el buffer contenía algo, se sustituye por el nuevo contenido.
 */
...
/*
 * Función String Concatenate, para añadir un texto a la cola de un buffer.
 * Si el buffer contenía algo, el nuevo contenido contenido se agrega detras.
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
   ...
   /*		     10 * percent					*/
    ...
    else if ( dampc <=   20 ) { SCOPY( vs, "araña" ); SCOPY( vp, "araña" ); }
    else if ( dampc <=   30 ) { SCOPY( vs, "daña" ); SCOPY( vp, "daña" ); }
...
//		gvflag = TRUE;
    ...
    if ( ch->skill_level[COMBAT_ABILITY] >= 15 )
       SCAT( buf2, " haces %d puntos de daño.", dam );
	if( dt != TYPE_MISSILE )
	{
...
{
    send_to_char( "Si quieres asesinar escríbelo entero.\n\r", ch );
    return;
}
...
void do_murder( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim == ch )
    {
	send_to_char( "Suicidarse está mal visto.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char("Eso es algo difícil ahora mismo...\n\r", ch);
	return;
    }
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	ch->in_room = was_in;
	act( AT_FLEE, "¡$n corre buscando protección!", ch, NULL, NULL, TO_ROOM );
	ch->in_room = now_in;
	act( AT_FLEE, "$n mira a su alrededor, comprobando si alguien le persigue.", ch, NULL, NULL, TO_ROOM );
        sprintf(buf, "¡Corres buscando protección!");
        send_to_char( buf, ch );
    ...
    sprintf(buf, "¡Intentas correr buscando protección!");
    send_to_char( buf, ch );
    return;
}
...
bool get_cover( CHAR_DATA *ch )
{
	...
	act( AT_FLEE, "¡$n corre buscando protección!", ch, NULL, NULL, TO_ROOM );
	ch->in_room = now_in;
	act( AT_FLEE, "$n corre en círculos.", ch, NULL, NULL, TO_ROOM );
...
const struct death_type hdeath_table[MAX_HDEATHS] = {
   ...
   {"La sangre sale a chorros del cuerpo de $n, ¡cubriéndote de pies a cabeza!"},
   {"Una mezcla de sangre y ácido estomacal salen del cuerpo de $n."},
   {"¡Las entrañas de $n se desparraman por el suelo llenándolo todo de sangre y vísceras!"}
};
...
const struct death_type ddeath_table[MAX_DDEATHS] = {
   ...
   {"¡$n explota en un montón de piezas!"}
};
