...
char * corpse_descs[] =
   { 
     "el cuerpo de %s pronto desaparecerá.", 
     "el cuerpo de %s yace aquí.",
     "El cuerpo de %s yace aquí.",
     "El cuerpo de %s yace aquí.",
     "El cuerpo de %s yace aquí."
   };
...
char * d_corpse_descs[] =
   { 
     "Los destrozados restos de %s pronto desaparecerán.", 
     "Los destrozados restos de %s están aquí.",
     "Los destrozados restos de %s están aquí.",
     "Los destrozados restos de %s están aquí.",
     "Los destrozados restos de %s están aquí."
   };
...
void gain_addiction( CHAR_DATA *ch )
{
    ...
    for ( drug=0 ; drug <= 9 ; drug ++ )
    {
       ...
       {      
           ch_printf ( ch, "Te gustaría consumir un poco de %s.\n\r",   spice_table[drug] );
           worsen_mental_state( ch, 1 );
       }
       else if ( ch->pcdata->addiction[drug] > ch->pcdata->drug_level[drug]+25 )
       {      
           ch_printf ( ch, "Algo más de %s sería agradable.\n\r",   spice_table[drug] );
       }
       else if ( ch->pcdata->addiction[drug] < ch->pcdata->drug_level[drug]-50 )
       {       
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	switch ( iCond )
	{
         case COND_FULL:
            if( ch->top_level <= LEVEL_IMMORTAL )
            {
               ...
               send_to_char( "¡Tu inanición comienza a disminuir tu salud!\n\r", ch );
	...
	case COND_FULL:
          ...
          {
            set_char_color( AT_HUNGRY, ch );
	    send_to_char( "Te apetecería picar algo.\n\r",  ch );
          } 
	  break;
	...
	case COND_THIRST:
          ...
          {
            set_char_color( AT_THIRSTY, ch );
	    send_to_char( "Te apetecería algo refrescante.\n\r", ch );
          } 
	  break;
...
void mobile_update( void )
{
	...
	if ( gch_prev && gch_prev->next != ch )
	{
	    ...
	    gch_prev = NULL;
	    ch->prev = NULL;
	    do_shout( ch, "Thoric dice, '¡Prepárate para lo peor!'" );
	}
...
/* Clean up 'animated corpses' that are not charmed' - Scryn */
	...
	if ( IS_SET(ch->act, ACT_MOUNTED ) )
	{
		...
		do_emote( ch, "gruñe y gruñe." );
	    continue;
	}

	if ( IS_SET(ch->in_room->room_flags, ROOM_SAFE )
	&&   IS_SET(ch->act, ACT_AGGRESSIVE) )
	    do_emote( ch, "mira a su alrededor y gruñe." );
	...
	&&   !IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) )
	{
		...
		if ( is_fearing(ch, rch) )
		{
		    ...
		    {
			case 0:
			  sprintf( buf, "¡Aléjate de mi, %s!", rch->name );
			  break;
			case 1:
			  sprintf( buf, "¡Déjame en paz, %s!", rch->name );
			  break;
			case 2:
			  sprintf( buf, "¡%s está intentando matarme! ¡Ayuda!", rch->name );
			  break;
			case 3:
...
void weather_update( void )
{
	...
	{
	    strcat( buf, "El cielo se está nublando." );
...
{   
	...
	if ( !char_died(ch) )
	{
		...
		switch( (ch->mental_state+5) / 10 )
		{
			...
			act( AT_ACTION, "Alguien debería ayudar a $n.", ch, NULL, NULL, TO_ROOM );
		    	break;
		    case  7:
			...
			act( AT_ACTION, "$n despotrica sobre cosas sin sentido...", ch, NULL, NULL, TO_ROOM );
		    	break;
		    case 10:
		    	send_to_char( "Sientes que el fin está cerca.\n\r", ch );
		...
		switch( (abs(ch->mental_state)+5) / 10 )
		{
			...
			if ( ch->position > POS_SLEEPING )
			{
			   ...
			   &&  (number_percent()+30) < abs(ch->mental_state) )
				...
				send_to_char( "Tienes muchísimo sueño.\n\r", ch );
			}
			break;
		    case   7:
		    ...
		    case   6:
			if ( ch->position > POS_RESTING )
			   send_to_char( "Necesitas un sueñecito.\n\r", ch );
			break;
		    case   5:
			if ( ch->position > POS_RESTING )
		    ...
		    case   3:
			if ( ch->position > POS_RESTING )
			   send_to_char( "Te gustaría tomar un descanso.\n\r", ch );
			break;
		}
            ...
            if ( !IS_NPC(ch) && time_info.hour == 1 && ch->pcdata->salary && ch->pcdata->clan && ch->pcdata->salary < ch->pcdata->clan->funds )
	    {
		  ...
		  ch_printf(ch, "&B[&YINformación bancaria&W: %s&B] &WTu salario de %d créditos ha sido depositado en tu cuenta.\n\r", ch->name, ch->pcdata->salary);
	    }
	    ...
	    if ( !IS_NPC(ch) && ch->pcdata->bank && (time_info.day == 5 || time_info.day == 15 || time_info.day == 25 || time_info.day == 10 || time_info.day == 20 || time_info.day == 30) && time_info.hour == 1)
	    {
		  ...
		  sprintf(buf, "&B[&YInformación bancaria&W: %s&B] &WTienes&W: &Y%ld&W intereses este mes.\n\r", ch->name, amount);
		  send_to_char(buf, ch);
	    }
              ...
              obj->value[3] != WEAPON_BOWCASTER &&  obj->value[3] != WEAPON_FORCE_PIKE)
        {
           ...
           if ( obj->value[4] <= 0 )
           {
              if ( obj->value[3] == WEAPON_LIGHTSABER )
              {
                 ...
                 act( AT_PLAIN, "El sable láser de $n se apaga." , obj->carried_by, NULL, NULL, TO_ROOM );
              }
	...
	if ( IS_OBJ_STAT(obj, ITEM_INVENTORY) )
	  continue;
        ...
        if ( obj->timer > 0 && obj->timer < 5 && obj->item_type == ITEM_ARMOR )
        {
          if ( obj->carried_by )
	  { 
	    act( AT_TEMP, "$p está medio muerto." , obj->carried_by, obj, NULL, TO_CHAR );
	  }
        }
...
void halucinations( CHAR_DATA *ch )
{
	...
	switch( number_range( 1, UMIN(20, (ch->mental_state+5) / 5)) )
	{
	    ...
	    case  2: t = "Te gustaría descansar.\n\r";				break;
	    case  3: t = "Deberías descansar.\n\r";					break;
	    case  4: t = "De repente sientes que algo está terriblemente mal.\n\r";	break;
	    case  5: t = "¡Alguien se ríe de ti!\n\r";		break;
	    case  6: t = "Oyes a tu madre llorar...\n\r";			break;
	    case  7: t = "¿Ya has estado aquí antes?  You're not sure...\n\r";	break;
	    case  8: t = "Por tu mente pasan recuerdos dolorosos de tu infancia.\n\r";	break;
	    case  9: t = "OYes a alguien gritar tu nombre en la distancia...\n\r";	break;
	    case 10: t = "Tu cabeza está latiendo ... no puedes pensar con claridad.\n\r";	break;
	    case 11: t = "El suelo... No se queda quieto...\n\r";			break;
	    case 12: t = "No distingues qué es real y que no.\n\r";		break;
	    case 13: t = "¿Esto es un sueño?\n\r";				break;
	    ...
	    case 18: t = "¡Estás en todas partes!\n\r";	break;
	    case 19: t = "¡Te sientes inmortal!\n\r";					break;
	    case 20: t = "Ah... El poder de una entidad suprema... ¿Qué hacer?\n\r";	break;
	}
	send_to_char( t, ch );
    }
    return;
...
{ 
    ...
    strcpy (log_buf, "Esperando autorización:\n\r" );
    for ( d = first_descriptor; d; d = d->next ) 
    {
...
void update_handler( void )
{
    ...
    if ( timechar )
    {
      ...
      send_to_char( "Iniciando el temporizador de actualización.\n\r", timechar );
...
//	do_who(NULL, "www");
    ...
    if ( timechar )
    {
      ...
      set_char_color(AT_PLAIN, timechar);
      send_to_char( "Temporizador de actualización finalizado.\n\r", timechar );
      subtract_times(&etime, &stime);
      ch_printf( timechar, "Tomó %d.%06d segundos.\n\r",
  ...
  if ( new_boot_time_t <= current_time )
  {
    ...
    if ( auction->item )
    {
      ...
      if ( auction->buyer && auction->buyer != auction->seller )
      {
        ...
        send_to_char("Tus créditos te han sido devueltos.\n\r", auction->buyer);
      }
    }
    echo_to_all(AT_AUCTION, "alguien te fuerza a esperar. El mundo se está reconstruyendo.", ECHOTAR_ALL);
...
/*
    ...
    if ( new_boot_time_t <= current_time )
    {
       ...
       if (auction->item != NULL)
       {
        ...
        {
            auction->buyer->gold += auction->bet;
            send_to_char ("Se te han devuelto los créditos.\n\r",auction->buyer);
        }
       }      
...
/*  if ( timecheck == 30 )*/
  ...
  { 
    if ( thirty == FALSE )
    {
	sprintf( buf, "Sientes una perturvación en la fuerza..." );
	echo_to_all( AT_YELLOW, buf, ECHOTAR_ALL );
	thirty = TRUE;
    }
    return;   
...
void auction_update (void)
{
	...
	case 2 : /* going twice */
		...
		sprintf (buf, "&W%s&w:&W a la &w%s&W por &Y%d&W créditos.", auction->item->short_descr,
	...
	case 3 : /* SOLD! */
	    ...
	    {
		sprintf (buf, "&W%s&W es &wvendido&W por &Y%d&W créditos.",
			auction->item->short_descr,
			auction->bet);
		talk_auction(buf);
		...
		sprintf(buf, "&wRecibes &Y%d&w créditos, quedándose como comisión el subastador &Y%d&w.\n\r", pay, tax);
		...
		boost_economy( auction->seller->in_room->area, tax );
		sprintf(buf, "&wTe cobran &Y%d&w créditos por las molestias.\n\r", tax );
		send_to_char(buf, auction->seller);
