...
void do_track( CHAR_DATA *ch, char *argument )
{
   ...
   if ( arg[0]=='\0' ) {
      send_to_char("¿A quién estás intentando rastrear?\n\r", ch);
      return;
   }
   ...
   if (!(vict = get_char_world(ch, arg))) {
      send_to_char("No encuentras su rastro.\n\r", ch);
      return;
   }

   if ( IS_AFFECTED( vict, AFF_COVER_TRAIL ))
   {
      send_to_char("No puedes encontrar su rastro.\n\r", ch);
      return;
   }
   ...
   switch(dir) {
      case BFS_ERROR:
         send_to_char("Hmm... Algo ha fallado.\n\r", ch);
         break;
      case BFS_ALREADY_THERE:
         send_to_char("¡Está frente a ti!\n\r", ch);
         break;
      case BFS_NO_PATH:
         sprintf(buf, "No puedes encontrar un camino desde aquí.\n\r" );
         send_to_char(buf, ch);
	 learn_from_failure( ch, gsn_track );
         break;
      default:
         ch_printf(ch, "sus huellas apuntan hacia %s de aquí...\n\r", dir_name3[dir]);
...
void found_prey( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	switch( number_bits( 2 ) )
 	{
	case 0: sprintf( buf, "No me hagas encontrarte, %s.", victname );
		do_say( ch, buf );
	        break;
	case 1: act( AT_ACTION, "$n mira a su alrededor buscando a $N.", ch, NULL, victim, TO_NOTVICT );
		...
		sprintf( buf, "¡Puedo oler tu sangre!" );
		do_say( ch, buf );
		break;
	case 2: sprintf( buf, "¡Te voy a matar, %s!", victname );
		do_yell( ch, buf );
		break;
	case 3: do_say( ch, "Espera a que te encuentre...");
		break;
        }
	return;
     }
	...
	switch( number_bits( 2 ) )
	{
	case 0:	do_say( ch, "¡Te voy a matar!" );
		sprintf( buf, "¡%s es un cobarde!", victname );
		do_yell( ch, buf );
		break;
	case 1: sprintf( buf, "¡Nos vemos fuera, %s!", victname );
		do_say( ch, buf );
		break;
	case 2: sprintf( buf, "¡%s es un cobarde!", victname );
		do_yell( ch, buf );
		break;
	case 3: act( AT_ACTION, "$n golpea a $N.", ch, NULL, victim, TO_NOTVICT );
		act( AT_ACTION, "Golpeas a $N.", ch, NULL, victim, TO_CHAR );
		act( AT_ACTION, "$n te golpea.", ch, NULL, victim, TO_VICT );
		break;
	}
	return;
     }
     ...
     switch( number_bits( 2 ) )
     {
     case 0: sprintf( buf, "¡Tu sangre es mía, %s!", victname );
	     do_yell( ch, buf);
	     break;
     case 1: sprintf( buf, "¡Nos volvemos a encontrar, %s!", victname );
     	     do_say( ch, buf );
     	     break;
     case 2: sprintf( buf, "¿Qué inscripción quieres en tu lápida, %s?", victname );
     	     do_say( ch, buf );
     	     break;
     case 3: act( AT_ACTION, "¡$n aparece tras $N!", ch, NULL, victim, TO_NOTVICT );
	     act( AT_ACTION, "¡Apareces tras $N!", ch, NULL, victim, TO_CHAR );
	     act( AT_ACTION, "¡$n aparece tras de ti!", ch, NULL, victim, TO_VICT );
     }
     stop_hunting( ch );
 ...
 {  
   ...
   if ( !pfound )
   {
    ...
    char_to_room( ch, victim->in_room );    
       ...
       sprintf( buf , "Un disparo bláster se dirige hacia ti desde %s." , dir_name3[dir] );
       act( AT_ACTION, buf , victim, NULL, ch, TO_CHAR );      
       act( AT_ACTION, "You fire at $N.", ch, NULL, victim, TO_CHAR );         
       sprintf( buf, "Un disparo bl´áster se dirige hacia $N desde %s." , dir_name3[dir] );
