...
/*
 * Función que reconoce los caracteres permitidos, incluyendo así
...
void read_from_buffer( DESCRIPTOR_DATA *d )
{
	...
	{
	    write_to_descriptor( d->descriptor, "Línea demasiado larga.\n\r", 0 );
...
/*    int iLang;*/
        ...
        if ( !str_cmp( argument, "New" ) )
	{
	    if (d->newstate == 0)
	    {
              ...
              /* Don't allow new players if DENY_NEW_PLAYERS is true */
      	      ...
      	      {
       		sprintf( buf, "El mud se está preparando para un reinicio.\n\r" );
		...
		close_socket( d, FALSE );
              ...
              sprintf( buf, "\n\rElejir un nombre es una de las partes más importantes del juego...\n\r"
              			"Asegúrate de que el nombre que elijas es adecuado para el personaje que vas\n\r"
               			"a interpretar, y que se adecúa a nuestra temática.\n\r"
               			"si el nombre que elijas no es válido, se te pedirá que\n\r"
	...
	if ( !d->character )
	{
	    ...
	    log_string( log_buf );
	    write_to_buffer( d, "tu ficha está corrupta. Notifica a los administradores.\n\r", 0 );
	    close_socket( d, FALSE );
	    return;
	}
	ch   = d->character;
	...
	else
	{
           ...
           if ( sysdata.wizlock == 1 && !IS_IMMORTAL(ch) )
            {
                write_to_buffer( d, "El juego está cerrado para mortales.\n\r", 0 );
                write_to_buffer( d, "Por favor prueba a entrar más tarde.\n\r", 0 );
                close_socket( d, FALSE );
                return;
            }
	}
...
//        bug(crypt(argument, ch->pcdata->pwd), 0);
        ...
        {
            write_to_buffer( d, "Contraseña incorrecta.\n\r", 0 );
            /* clear descriptor pointer to get rid of bug message in log */
	...
	{
	case 's': case 'S':
	    sprintf( buf, "\n\r&zpor favor teclea una contraseña que nadie pueda adivinar."
	    		  "\n\rElije una buena contraseña para %s: &w%s",
	...
	case 'n': case 'N':
	    send_to_desc_color2("&zVale, ¿cómo te llamas entonces? &w", d );
	    /* clear descriptor pointer to get rid of bug message in log */
	    d->character->desc = NULL;
	    free_char( d->character );
	    d->character = NULL;
	...
	{
	    send_to_desc_color2("&zla contraseña debe tener al menos cinco caracteres de longitud.\n\rPassword: &w",d );
	    return;
	}
	...
	for ( p = pwdnew; *p != '\0'; p++ )
	{
	    if ( *p == '~' )
	    {
		send_to_desc_color2( "&zLa contraseña no es aceptable, prueba de nuevo.\n\rPassword: &w",d );
		return;
	    }
	}
	...
	send_to_desc_color2("\n\r&zPor favor teclea nuevamente la contraseña para confirmar: &w", d );
	...
	if ( strcmp( smaug_crypt( argument ), ch->pcdata->pwd ) )
	{
	    send_to_desc_color2("&zLas contraseñas no coinciden.\n\rTeclea nuevamente la contraseña: &w",d);
	    d->connected = CON_GET_NEW_PASSWORD;
	    return;
	}

	write_to_buffer( d, echo_on_str, 0 );
	send_to_desc_color2( "\n\r&z¿Cuál es tu sexo (&WM&z/&WF&z/&WN&z)? &w", d);
	...
	default:
	    send_to_desc_color2("&zEso no es un sexo.\n\r¿Cuál es tu sexo?&w ", d );
	    return;
	}

	send_to_desc_color2("\n\r&zelije una de las razas siguientes, o teclea &Wayuda [raza]&z para aprender más.\n\r", d );
	send_to_desc_color2("Ten en cuenta que debes interpretar la raza que elijas. Si elijes\n\r", d);
	send_to_desc_color2("un droide, deberás interpretar tu personaje en base a esto. Si elijes un humano,\n\r", d);
	send_to_desc_color2("deberás interpretar tu personaje como se espera de esta raza. Esto aplica para todas\n\r", d);
	send_to_desc_color2("las razas, si no estás seguro por favor consulta la ayuda de la raza.\n\r", d);
	send_to_desc_color2("Si después de eso sigues sin estar seguro, por favor, no elijas esa raza.\n\r\n\r", d);
	buf[0] = '\0';
	col=0;
	...
	sprintf( buf, "\n\r&zFUE: &R%d  &zINT: &R%d  &zSAB: &R%d  &zDES: &R%d  &zCON: &R%d  &zCAR: &R%d\n\r" ,
        ...
        send_to_desc_color2( buf, d );
        send_to_desc_color2( "\n\r&z¿Te gustan estas características?&w ", d );
	...
	case 'n': case 'N': 
	    ...
	    ch->perm_dex, ch->perm_con, ch->perm_cha) ;
            ...
            send_to_desc_color2( buf, d );
            send_to_desc_color2( "\n\r&z¿Estas te gustan más?&w ", d );
	    return;
	default:
	    send_to_desc_color2( "&zSelección inválida.\n\r¿&WSi&z o &WNO&z? ", d );
	    return;
	}
 ...
 {
	send_to_desc_color2("\n\r&zPor favor elije la descripción que más se ajuste a tu personaje:&w\n\r", d );
	buf[0] = '\0';
	col = 0;
   ...
   if(!IS_DROID(ch))
	...
	if (iClass == 4 ||  !height_name[iClass] || height_name[iClass][0] == '\0')
        {
            send_to_desc_color2( "&zEso no es una atura.\n\r¿cómo quieres ser?&w ", d );
            return;
        }

	send_to_desc_color2("\n\r&zPor favor, elije un tipo de complexión:&w\n\r", d );
    ...
    if( iClass == 6 || !build_name[iClass] || build_name[iClass][0] == '\0')
        {
            send_to_desc_color2( "&zEso no es una complexión.\n\r¿cómo quieres ser?&w ", d );
            return;
        }
   ...
   if(IS_DROID(ch))
   {
	...
	if (iDroid == 8 ||  !droid_name[iDroid] || droid_name[iDroid][0] == '\0')
        {
            send_to_desc_color2( "&zEso no es una descripción.\n\r¿cómo quieres ser?&w ", d );
            return;
        }
   }
/*  Changing this up a bit...automatically sets PLR_ANSI, skips want ansi/msp.

	write_to_buffer( d, "\n\r¿Te gustaría jugar con ANSI? La alternativa es no gráficos/soporte de color, (A/N)? ", 0 ); */
...
/*	if ( IS_SET(ch->act, PLR_ANSI) )
	...
	if ( ch->top_level >= 0 )
	{
	  send_to_pager( "\n\r&WInformación importante&w\n\r", ch );
	  do_help( ch, "motd" );
	}
/* Uh, do we use this?
	if ( IS_IMMORTAL(ch) )
	{
	  send_to_pager( "&WInformación importante para inmortales&w\n\r", ch );
	  do_help( ch, "imotd" );
	}
*/
	send_to_pager( "\n\r&WPulsa [ENTER] &w", ch );
...
ch->speaking	=  iLang;
	    	/* anulamos momentáneamente los idiomas
...
bool check_reconnect( DESCRIPTOR_DATA *d, char *name, bool fConn )
{
	...
	&&   !str_cmp( name, ch->name ) )
	{
	    ...
	    {
	      write_to_buffer( d, "Ya está jugando.\n\rNombre: ", 0 );
	      d->connected = CON_GET_NAME;
	      if ( d->character )
	      {
		 /* clear descriptor pointer to get rid of bug message in log */
...
bool check_multi( DESCRIPTOR_DATA *d , char *name )
{
        ...
        && !str_cmp(dold->host , d->host ) )
	{
		...
		for ( iloop = 0 ; iloop < 11 ; iloop++ )
	        ...
	        if ( iloop >= 10 )
	           return FALSE; 
		write_to_buffer( d, "Lo sentimos, entrar con varios personajes a la vez no está permitido...\n\r", 0 );
...
bool check_playing( DESCRIPTOR_DATA *d, char *name, bool kick )
{
		 ...
		 ? dold->original->name : dold->character->name ) )
	{
	    ...
	    {
		write_to_buffer( d, "ya está jugando - prueba otra vez.\n\r", 0 );
	    ...
	    if ( !kick )
	      return TRUE;
	    write_to_buffer( d, "ya estás jugando... Desconectando conexión anterior.\n\r", 0 );
	    write_to_buffer( dold, "Desconectando por conexión nueva... ¡adiós!\n\r", 0 );
	    ...
	    ch->switched = NULL;
	    send_to_char( "Reconectando.\n\r", ch );
	    act( AT_ACTION, "$n ha reconectado, desconectando su conexión anterior.",
...
void do_name( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char("tu nombre ha sido cambiado.  Por favor espera la autorización.\n\r", ch);
