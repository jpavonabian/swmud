...
void start_editing_nolimit( CHAR_DATA *ch, char *old_text, sh_int max_total )
{
	...
	send_to_char( "Teclea un texto (/? = ayuda /s = salir guardando los cambios /c = borrar todo /l = mostrar todas las líneas)\n\r", ch );
	send_to_char( "-----------------------------------------------------------------------\n\r", ch );
	if ( ch->editor )
	  stop_editing( ch );
...
void edit_buffer( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( cmd+1, "?" ) )
		...
		send_to_char( "¿Uh? Teclea '/?' para consultar la lista de comandos válidos.\n\r", ch );
	...
	*p = '\0';
    ...
    {
	send_to_char( "No puedes escribir más.\n\r", ch );
	editor_save( ch, edd, "");
    }
    else
    {
...
void editor_help( CHAR_DATA *ch, EDITOR_DATA *edd, char *argument )
{
    ...
    if( arg[i] == NULL )
	send_to_char( "No existe esa ayuda.\n\r", ch );
    else
...
void editor_search_and_replace( CHAR_DATA *ch, EDITOR_DATA *edd, char *argument )
{
    ...
    {
	send_to_char( "Necesaria la cadena a reemplazar y la cadena reemplazadaora.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Operación imposible de realizar por tamaño.\n\r", ch );
	discard_editdata( cloned_edd );
    }
    else
    {
    	ch_printf( ch, "Reemplazando todas las ocurriencias de '%s' con '%s'...\n\r", word_src, word_dst );
	discard_editdata( edd );
	ch->editor = cloned_edd;
    	ch_printf( ch, "encontradas y reemplazadas %d cadena(s).\n\r", repl_count );
    }
