...
void save_protocol( void );

extern void init_descriptor( DESCRIPTOR_DATA *dnew, int desc);
   ...
   for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
   {
      for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
      {
         if( pRoomIndex )
         {
            ...
            snprintf( filename, 256, "%s%d", COPYOVER_DIR, pRoomIndex->vnum );
...
void load_obj_files( void )
{
   ...
   snprintf( directory_name, 100, "%s", COPYOVER_DIR );
...
void do_copyover( CHAR_DATA * ch, char *argument )
{
   ...
   for( d = first_descriptor; d; d = de_next )
   {
      ...
      {
         write_to_descriptor( d->descriptor, "\r\nLo sentimos, estamos reiniciando. vuelve en unos minutos.\r\n", 0 );
...
/*      d->next = NULL;
      ...
      {
         write_to_descriptor( d->descriptor, "al final no ocurre nada. todo continúa como siempre.\r\n", 0 );
         ...
         act( AT_MAGIC, "¡Apareces!", d->character, NULL, NULL, TO_CHAR );
         act( AT_MAGIC, "¡$n aparece!", d->character, NULL, NULL, TO_ROOM );
   ...
   log_string( "Reinicio completado." );
   return;
}
...
void emergency_copyover( void )
{
  ...
  char buf[MSL], buf2[100], buf3[100], buf6[100];
  extern int control;
       ...
       if (!d->character || d->connected < CON_PLAYING) /* drop those logging on */
         {
        ...
        else
          {
            ...
            save_char_obj (och);
                   ...
                   write_to_descriptor( d->descriptor, buf, 0);
                   write_to_descriptor( d->descriptor, buf6, 0 );
                   compressEnd( d );
         /*
          * One of two places this gets changed
          */
 ...
 /* Close reserve and other always-open files and release other resources */
  ...
  fclose (fpLOG);
   ...
   log_string( "Executing Emergency Copyover...." );
...
void forcesave( void )
{
   ...
   for( vch = first_char; vch; vch = vch_next )
   {
      ...
      if( !IS_NPC( vch ) )
      {
         send_to_char( "Emergency...You are forced to save!\n\r", vch );
         do_save( vch, "" );
      }
   }
